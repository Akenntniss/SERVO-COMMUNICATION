...
CREATE INDEX IF NOT EXISTS idx_sms_history_retry_count ON sms_history(retry_count);

-- Ajouter un index composé pour les requêtes de timeout
CREATE INDEX IF NOT EXISTS idx_sms_history_status_created ON sms_history(status, created_at);
