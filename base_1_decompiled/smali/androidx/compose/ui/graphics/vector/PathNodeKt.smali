.class public final Landroidx/compose/ui/graphics/vector/PathNodeKt;
.super Ljava/lang/Object;
.source "PathNode.kt"


# annotations
.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nPathNode.kt\nKotlin\n*S Kotlin\n*F\n+ 1 PathNode.kt\nandroidx/compose/ui/graphics/vector/PathNodeKt\n+ 2 _Collections.kt\nkotlin/collections/CollectionsKt___CollectionsKt\n*L\n1#1,333:1\n283#1:334\n284#1,9:338\n283#1:348\n284#1,9:352\n283#1:362\n284#1,9:366\n283#1:376\n284#1,9:380\n283#1:390\n284#1,9:394\n283#1:404\n284#1,9:408\n283#1:418\n284#1,9:422\n283#1:432\n284#1,9:436\n283#1:446\n284#1,9:450\n283#1:460\n284#1,9:464\n283#1:474\n284#1,9:478\n283#1:488\n284#1,9:492\n283#1:502\n284#1,9:506\n283#1:516\n284#1,9:520\n283#1:530\n284#1,9:534\n283#1:544\n284#1,9:548\n283#1:558\n284#1,9:562\n283#1:572\n284#1,9:576\n1547#2:335\n1618#2,2:336\n1620#2:347\n1547#2:349\n1618#2,2:350\n1620#2:361\n1547#2:363\n1618#2,2:364\n1620#2:375\n1547#2:377\n1618#2,2:378\n1620#2:389\n1547#2:391\n1618#2,2:392\n1620#2:403\n1547#2:405\n1618#2,2:406\n1620#2:417\n1547#2:419\n1618#2,2:420\n1620#2:431\n1547#2:433\n1618#2,2:434\n1620#2:445\n1547#2:447\n1618#2,2:448\n1620#2:459\n1547#2:461\n1618#2,2:462\n1620#2:473\n1547#2:475\n1618#2,2:476\n1620#2:487\n1547#2:489\n1618#2,2:490\n1620#2:501\n1547#2:503\n1618#2,2:504\n1620#2:515\n1547#2:517\n1618#2,2:518\n1620#2:529\n1547#2:531\n1618#2,2:532\n1620#2:543\n1547#2:545\n1618#2,2:546\n1620#2:557\n1547#2:559\n1618#2,2:560\n1620#2:571\n1547#2:573\n1618#2,2:574\n1620#2:585\n1547#2:586\n1618#2,3:587\n*S KotlinDebug\n*F\n+ 1 PathNode.kt\nandroidx/compose/ui/graphics/vector/PathNodeKt\n*L\n153#1:334\n153#1:338,9\n157#1:348\n157#1:352,9\n161#1:362\n161#1:366,9\n165#1:376\n165#1:380,9\n169#1:390\n169#1:394,9\n173#1:404\n173#1:408,9\n177#1:418\n177#1:422,9\n181#1:432\n181#1:436,9\n185#1:446\n185#1:450,9\n196#1:460\n196#1:464,9\n207#1:474\n207#1:478,9\n216#1:488\n216#1:492,9\n225#1:502\n225#1:506,9\n234#1:516\n234#1:520,9\n243#1:530\n243#1:534,9\n247#1:544\n247#1:548,9\n251#1:558\n251#1:562,9\n263#1:572\n263#1:576,9\n153#1:335\n153#1:336,2\n153#1:347\n157#1:349\n157#1:350,2\n157#1:361\n161#1:363\n161#1:364,2\n161#1:375\n165#1:377\n165#1:378,2\n165#1:389\n169#1:391\n169#1:392,2\n169#1:403\n173#1:405\n173#1:406,2\n173#1:417\n177#1:419\n177#1:420,2\n177#1:431\n181#1:433\n181#1:434,2\n181#1:445\n185#1:447\n185#1:448,2\n185#1:459\n196#1:461\n196#1:462,2\n196#1:473\n207#1:475\n207#1:476,2\n207#1:487\n216#1:489\n216#1:490,2\n216#1:501\n225#1:503\n225#1:504,2\n225#1:515\n234#1:517\n234#1:518,2\n234#1:529\n243#1:531\n243#1:532,2\n243#1:543\n247#1:545\n247#1:546,2\n247#1:557\n251#1:559\n251#1:560,2\n251#1:571\n263#1:573\n263#1:574,2\n263#1:585\n283#1:586\n283#1:587,3\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u00000\n\u0000\n\u0002\u0010\u000c\n\u0002\u0008\u0006\n\u0002\u0010\u0008\n\u0002\u0008\u0017\n\u0002\u0010 \n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0014\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0004\u001aB\u0010\u001f\u001a\u0008\u0012\u0004\u0012\u00020!0 2\u0006\u0010\"\u001a\u00020#2\u0006\u0010$\u001a\u00020\u00082!\u0010%\u001a\u001d\u0012\u0013\u0012\u00110#\u00a2\u0006\u000c\u0008\'\u0012\u0008\u0008(\u0012\u0004\u0008\u0008()\u0012\u0004\u0012\u00020!0&H\u0082\u0008\u001a\u001a\u0010*\u001a\u0008\u0012\u0004\u0012\u00020!0 *\u00020\u00012\u0006\u0010\"\u001a\u00020#H\u0000\"\u000e\u0010\u0000\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0002\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0003\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0004\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0005\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0006\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0007\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\t\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\n\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u000b\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u000c\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\r\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u000e\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u000f\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0010\u001a\u00020\u0008X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0011\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0012\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0013\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0014\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0015\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0016\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0017\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0018\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u0019\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u001a\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u001b\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u001c\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u001d\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\"\u000e\u0010\u001e\u001a\u00020\u0001X\u0082T\u00a2\u0006\u0002\n\u0000\u00a8\u0006+"
    }
    d2 = {
        "ArcToKey",
        "",
        "CloseKey",
        "CurveToKey",
        "HorizontalToKey",
        "LineToKey",
        "MoveToKey",
        "NUM_ARC_TO_ARGS",
        "",
        "NUM_CURVE_TO_ARGS",
        "NUM_HORIZONTAL_TO_ARGS",
        "NUM_LINE_TO_ARGS",
        "NUM_MOVE_TO_ARGS",
        "NUM_QUAD_TO_ARGS",
        "NUM_REFLECTIVE_CURVE_TO_ARGS",
        "NUM_REFLECTIVE_QUAD_TO_ARGS",
        "NUM_VERTICAL_TO_ARGS",
        "QuadToKey",
        "ReflectiveCurveToKey",
        "ReflectiveQuadToKey",
        "RelativeArcToKey",
        "RelativeCloseKey",
        "RelativeCurveToKey",
        "RelativeHorizontalToKey",
        "RelativeLineToKey",
        "RelativeMoveToKey",
        "RelativeQuadToKey",
        "RelativeReflectiveCurveToKey",
        "RelativeReflectiveQuadToKey",
        "RelativeVerticalToKey",
        "VerticalToKey",
        "pathNodesFromArgs",
        "",
        "Landroidx/compose/ui/graphics/vector/PathNode;",
        "args",
        "",
        "numArgs",
        "nodeFor",
        "Lkotlin/Function1;",
        "Lkotlin/ParameterName;",
        "name",
        "subArray",
        "toPathNodes",
        "ui-graphics_release"
    }
    k = 0x2
    mv = {
        0x1,
        0x5,
        0x1
    }
    xi = 0x30
.end annotation


# static fields
.field private static final ArcToKey:C = 'A'

.field private static final CloseKey:C = 'Z'

.field private static final CurveToKey:C = 'C'

.field private static final HorizontalToKey:C = 'H'

.field private static final LineToKey:C = 'L'

.field private static final MoveToKey:C = 'M'

.field private static final NUM_ARC_TO_ARGS:I = 0x7

.field private static final NUM_CURVE_TO_ARGS:I = 0x6

.field private static final NUM_HORIZONTAL_TO_ARGS:I = 0x1

.field private static final NUM_LINE_TO_ARGS:I = 0x2

.field private static final NUM_MOVE_TO_ARGS:I = 0x2

.field private static final NUM_QUAD_TO_ARGS:I = 0x4

.field private static final NUM_REFLECTIVE_CURVE_TO_ARGS:I = 0x4

.field private static final NUM_REFLECTIVE_QUAD_TO_ARGS:I = 0x2

.field private static final NUM_VERTICAL_TO_ARGS:I = 0x1

.field private static final QuadToKey:C = 'Q'

.field private static final ReflectiveCurveToKey:C = 'S'

.field private static final ReflectiveQuadToKey:C = 'T'

.field private static final RelativeArcToKey:C = 'a'

.field private static final RelativeCloseKey:C = 'z'

.field private static final RelativeCurveToKey:C = 'c'

.field private static final RelativeHorizontalToKey:C = 'h'

.field private static final RelativeLineToKey:C = 'l'

.field private static final RelativeMoveToKey:C = 'm'

.field private static final RelativeQuadToKey:C = 'q'

.field private static final RelativeReflectiveCurveToKey:C = 's'

.field private static final RelativeReflectiveQuadToKey:C = 't'

.field private static final RelativeVerticalToKey:C = 'v'

.field private static final VerticalToKey:C = 'V'


# direct methods
.method private static final pathNodesFromArgs([FILkotlin/jvm/functions/Function1;)Ljava/util/List;
    .locals 18
    .param p0, "args"    # [F
    .param p1, "numArgs"    # I
    .param p2, "nodeFor"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "([FI",
            "Lkotlin/jvm/functions/Function1<",
            "-[F+",
            "Landroidx/compose/ui/graphics/vector/PathNode;",
            ">;)",
            "Ljava/util/List<",
            "Landroidx/compose/ui/graphics/vector/PathNode;",
            ">;"
        }
    .end annotation

    move-object/from16 v0, p0

    move/from16 v1, p1

    const/4 v2, 0x0

    .line 283
    .local v2, "$i$f$pathNodesFromArgs":I
    new-instance v3, Lkotlin/ranges/IntRange;

    array-length v4, v0

    sub-int/2addr v4, v1

    const/4 v5, 0x0

    invoke-direct {v3, v5, v4}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v3, Lkotlin/ranges/IntProgression;

    invoke-static {v3, v1}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v3

    check-cast v3, Ljava/lang/Iterable;

    .local v3, "$this$map$iv":Ljava/lang/Iterable;
    const/4 v4, 0x0

    .line 586
    .local v4, "$i$f$map":I
    new-instance v6, Ljava/util/ArrayList;

    const/16 v7, 0xa

    invoke-static {v3, v7}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v7

    invoke-direct {v6, v7}, Ljava/util/ArrayList;-><init>(I)V

    check-cast v6, Ljava/util/Collection;

    .local v6, "destination$iv$iv":Ljava/util/Collection;
    move-object v7, v3

    .local v7, "$this$mapTo$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 587
    .local v8, "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_0
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_2

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 588
    .local v10, "item$iv$iv":I
    move v11, v10

    .local v11, "index":I
    const/4 v12, 0x0

    .line 284
    .local v12, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1":I
    add-int v13, v11, v1

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v0, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 285
    .local v13, "subArray":[F
    move-object/from16 v14, p2

    invoke-interface {v14, v13}, Lkotlin/jvm/functions/Function1;->invoke(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v15

    check-cast v15, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 286
    .local v15, "node":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 289
    instance-of v5, v15, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    const/16 v17, 0x1

    if-eqz v5, :cond_0

    if-lez v11, :cond_0

    new-instance v5, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/16 v16, 0x0

    aget v0, v13, v16

    aget v1, v13, v17

    invoke-direct {v5, v0, v1}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v5, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1

    .line 290
    :cond_0
    instance-of v0, v15, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v0, :cond_1

    if-lez v11, :cond_1

    .line 291
    new-instance v0, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v1, 0x0

    aget v5, v13, v1

    aget v1, v13, v17

    invoke-direct {v0, v5, v1}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    move-object v5, v0

    check-cast v5, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1

    .line 292
    :cond_1
    move-object v5, v15

    .line 286
    :goto_1
    nop

    .end local v11    # "index":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1":I
    .end local v13    # "subArray":[F
    .end local v15    # "node":Landroidx/compose/ui/graphics/vector/PathNode;
    invoke-interface {v6, v5}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 587
    .end local v10    # "item$iv$iv":I
    const/4 v5, 0x0

    move-object/from16 v0, p0

    move/from16 v1, p1

    goto :goto_0

    .line 589
    :cond_2
    move-object/from16 v14, p2

    .end local v6    # "destination$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    move-object v0, v6

    check-cast v0, Ljava/util/List;

    .line 586
    nop

    .line 283
    .end local v3    # "$this$map$iv":Ljava/lang/Iterable;
    .end local v4    # "$i$f$map":I
    return-object v0
.end method

.method public static final toPathNodes(C[F)Ljava/util/List;
    .locals 34
    .param p0, "$this$toPathNodes"    # C
    .param p1, "args"    # [F
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(C[F)",
            "Ljava/util/List<",
            "Landroidx/compose/ui/graphics/vector/PathNode;",
            ">;"
        }
    .end annotation

    move/from16 v0, p0

    move-object/from16 v1, p1

    const-string v2, "args"

    invoke-static {v1, v2}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 151
    nop

    .line 152
    const/16 v2, 0x7a

    const/4 v3, 0x1

    const/4 v4, 0x0

    if-ne v0, v2, :cond_0

    :goto_0
    move v2, v3

    goto :goto_1

    :cond_0
    const/16 v2, 0x5a

    if-ne v0, v2, :cond_1

    goto :goto_0

    :cond_1
    move v2, v4

    :goto_1
    if-eqz v2, :cond_2

    sget-object v2, Landroidx/compose/ui/graphics/vector/PathNode$Close;->INSTANCE:Landroidx/compose/ui/graphics/vector/PathNode$Close;

    invoke-static {v2}, Lkotlin/collections/CollectionsKt;->listOf(Ljava/lang/Object;)Ljava/util/List;

    move-result-object v2

    goto/16 :goto_2a

    .line 153
    :cond_2
    const/16 v2, 0x6d

    const/16 v5, 0xa

    if-ne v0, v2, :cond_6

    const/4 v2, 0x2

    .local v2, "numArgs$iv":I
    const/4 v6, 0x0

    .line 334
    .local v6, "$i$f$pathNodesFromArgs":I
    new-instance v7, Lkotlin/ranges/IntRange;

    array-length v8, v1

    sub-int/2addr v8, v2

    invoke-direct {v7, v4, v8}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v7, Lkotlin/ranges/IntProgression;

    invoke-static {v7, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v7

    check-cast v7, Ljava/lang/Iterable;

    .local v7, "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 335
    .local v8, "$i$f$map":I
    new-instance v9, Ljava/util/ArrayList;

    invoke-static {v7, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v9, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v9

    check-cast v5, Ljava/util/Collection;

    .local v5, "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v9, v7

    .local v9, "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v10, 0x0

    .line 336
    .local v10, "$i$f$mapTo":I
    invoke-interface {v9}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v11

    :goto_2
    invoke-interface {v11}, Ljava/util/Iterator;->hasNext()Z

    move-result v12

    if-eqz v12, :cond_5

    move-object v12, v11

    check-cast v12, Lkotlin/collections/IntIterator;

    invoke-virtual {v12}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v12

    .line 337
    .local v12, "item$iv$iv$iv":I
    move v13, v12

    .local v13, "index$iv":I
    const/4 v14, 0x0

    .line 338
    .local v14, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v15, v13, v2

    invoke-static {v13, v15}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v15

    invoke-static {v1, v15}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v15

    check-cast v15, Ljava/util/Collection;

    invoke-static {v15}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v15

    .line 339
    .local v15, "subArray$iv":[F
    move-object/from16 v16, v15

    .local v16, "array":[F
    const/16 v17, 0x0

    .line 154
    .local v17, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$1":I
    move/from16 v18, v2

    .end local v2    # "numArgs$iv":I
    .local v18, "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    move/from16 v19, v6

    .end local v6    # "$i$f$pathNodesFromArgs":I
    .local v19, "$i$f$pathNodesFromArgs":I
    aget v6, v16, v4

    aget v4, v16, v3

    invoke-direct {v2, v6, v4}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;-><init>(FF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 340
    .end local v16    # "array":[F
    .end local v17    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$1":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 343
    instance-of v4, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v4, :cond_3

    if-lez v13, :cond_3

    new-instance v4, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    move-object/from16 v16, v2

    const/4 v6, 0x0

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .local v16, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    aget v2, v15, v6

    aget v6, v15, v3

    invoke-direct {v4, v2, v6}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_3

    .end local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    :cond_3
    move-object/from16 v16, v2

    .line 344
    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    if-lez v13, :cond_4

    .line 345
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v6, v15, v4

    aget v4, v15, v3

    invoke-direct {v2, v6, v4}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    move-object v4, v2

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_3

    .line 346
    :cond_4
    move-object/from16 v4, v16

    .line 340
    :goto_3
    nop

    .end local v13    # "index$iv":I
    .end local v14    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v15    # "subArray$iv":[F
    .end local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    invoke-interface {v5, v4}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 336
    .end local v12    # "item$iv$iv$iv":I
    move/from16 v2, v18

    move/from16 v6, v19

    const/4 v4, 0x0

    goto :goto_2

    .line 347
    .end local v18    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v6    # "$i$f$pathNodesFromArgs":I
    :cond_5
    move/from16 v18, v2

    move/from16 v19, v6

    .end local v2    # "numArgs$iv":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$i$f$pathNodesFromArgs":I
    .end local v9    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v10    # "$i$f$mapTo":I
    .restart local v18    # "numArgs$iv":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 335
    nop

    .line 334
    .end local v7    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$map":I
    nop

    .end local v18    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 157
    :cond_6
    const/16 v2, 0x4d

    if-ne v0, v2, :cond_a

    const/4 v2, 0x2

    .restart local v2    # "numArgs$iv":I
    const/4 v4, 0x0

    .line 348
    .local v4, "$i$f$pathNodesFromArgs":I
    new-instance v6, Lkotlin/ranges/IntRange;

    array-length v7, v1

    sub-int/2addr v7, v2

    const/4 v8, 0x0

    invoke-direct {v6, v8, v7}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v6, Lkotlin/ranges/IntProgression;

    invoke-static {v6, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v6

    check-cast v6, Ljava/lang/Iterable;

    .local v6, "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v7, 0x0

    .line 349
    .local v7, "$i$f$map":I
    new-instance v8, Ljava/util/ArrayList;

    invoke-static {v6, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v8, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v8

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v8, v6

    .local v8, "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v9, 0x0

    .line 350
    .local v9, "$i$f$mapTo":I
    invoke-interface {v8}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v10

    :goto_4
    invoke-interface {v10}, Ljava/util/Iterator;->hasNext()Z

    move-result v11

    if-eqz v11, :cond_9

    move-object v11, v10

    check-cast v11, Lkotlin/collections/IntIterator;

    invoke-virtual {v11}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v11

    .line 351
    .local v11, "item$iv$iv$iv":I
    move v12, v11

    .local v12, "index$iv":I
    const/4 v13, 0x0

    .line 352
    .local v13, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v14, v12, v2

    invoke-static {v12, v14}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v14

    invoke-static {v1, v14}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v14

    check-cast v14, Ljava/util/Collection;

    invoke-static {v14}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v14

    .line 353
    .local v14, "subArray$iv":[F
    move-object v15, v14

    .local v15, "array":[F
    const/16 v16, 0x0

    .line 158
    .local v16, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$2":I
    move/from16 v17, v2

    .end local v2    # "numArgs$iv":I
    .local v17, "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    move/from16 v19, v4

    const/16 v18, 0x0

    .end local v4    # "$i$f$pathNodesFromArgs":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    aget v4, v15, v18

    move-object/from16 v21, v6

    .end local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .local v21, "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v6, v15, v3

    invoke-direct {v2, v4, v6}, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;-><init>(FF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 354
    .end local v15    # "array":[F
    .end local v16    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$2":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 357
    if-lez v12, :cond_7

    new-instance v4, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    aget v6, v14, v18

    aget v15, v14, v3

    invoke-direct {v4, v6, v15}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_5

    .line 358
    :cond_7
    instance-of v4, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v4, :cond_8

    if-lez v12, :cond_8

    .line 359
    new-instance v4, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v6, 0x0

    aget v15, v14, v6

    aget v6, v14, v3

    invoke-direct {v4, v15, v6}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_5

    .line 360
    :cond_8
    move-object v4, v2

    .line 354
    :goto_5
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v12    # "index$iv":I
    .end local v13    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v14    # "subArray$iv":[F
    invoke-interface {v5, v4}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 350
    .end local v11    # "item$iv$iv$iv":I
    move/from16 v2, v17

    move/from16 v4, v19

    move-object/from16 v6, v21

    goto :goto_4

    .line 361
    .end local v17    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .end local v21    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .local v2, "numArgs$iv":I
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    .restart local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_9
    move/from16 v17, v2

    move/from16 v19, v4

    move-object/from16 v21, v6

    .end local v2    # "numArgs$iv":I
    .end local v4    # "$i$f$pathNodesFromArgs":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v9    # "$i$f$mapTo":I
    .restart local v17    # "numArgs$iv":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    .restart local v21    # "$this$map$iv$iv":Ljava/lang/Iterable;
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 349
    nop

    .line 348
    .end local v7    # "$i$f$map":I
    .end local v21    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v17    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 161
    :cond_a
    const/16 v2, 0x6c

    if-ne v0, v2, :cond_e

    const/4 v2, 0x2

    .restart local v2    # "numArgs$iv":I
    const/4 v4, 0x0

    .line 362
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    new-instance v6, Lkotlin/ranges/IntRange;

    array-length v7, v1

    sub-int/2addr v7, v2

    const/4 v8, 0x0

    invoke-direct {v6, v8, v7}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v6, Lkotlin/ranges/IntProgression;

    invoke-static {v6, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v6

    check-cast v6, Ljava/lang/Iterable;

    .restart local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v7, 0x0

    .line 363
    .restart local v7    # "$i$f$map":I
    new-instance v8, Ljava/util/ArrayList;

    invoke-static {v6, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v8, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v8

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v8, v6

    .restart local v8    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v9, 0x0

    .line 364
    .restart local v9    # "$i$f$mapTo":I
    invoke-interface {v8}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v10

    :goto_6
    invoke-interface {v10}, Ljava/util/Iterator;->hasNext()Z

    move-result v11

    if-eqz v11, :cond_d

    move-object v11, v10

    check-cast v11, Lkotlin/collections/IntIterator;

    invoke-virtual {v11}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v11

    .line 365
    .restart local v11    # "item$iv$iv$iv":I
    move v12, v11

    .restart local v12    # "index$iv":I
    const/4 v13, 0x0

    .line 366
    .restart local v13    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v14, v12, v2

    invoke-static {v12, v14}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v14

    invoke-static {v1, v14}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v14

    check-cast v14, Ljava/util/Collection;

    invoke-static {v14}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v14

    .line 367
    .restart local v14    # "subArray$iv":[F
    move-object v15, v14

    .restart local v15    # "array":[F
    const/16 v16, 0x0

    .line 162
    .local v16, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$3":I
    move/from16 v17, v2

    .end local v2    # "numArgs$iv":I
    .restart local v17    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    move/from16 v19, v4

    const/16 v18, 0x0

    .end local v4    # "$i$f$pathNodesFromArgs":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    aget v4, v15, v18

    move-object/from16 v18, v6

    .end local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .local v18, "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v6, v15, v3

    invoke-direct {v2, v4, v6}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 368
    .end local v15    # "array":[F
    .end local v16    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$3":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 371
    instance-of v4, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v4, :cond_b

    if-lez v12, :cond_b

    new-instance v4, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v6, 0x0

    aget v15, v14, v6

    aget v6, v14, v3

    invoke-direct {v4, v15, v6}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_7

    .line 372
    :cond_b
    instance-of v4, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v4, :cond_c

    if-lez v12, :cond_c

    .line 373
    new-instance v4, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v6, 0x0

    aget v15, v14, v6

    aget v6, v14, v3

    invoke-direct {v4, v15, v6}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_7

    .line 374
    :cond_c
    move-object v4, v2

    .line 368
    :goto_7
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v12    # "index$iv":I
    .end local v13    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v14    # "subArray$iv":[F
    invoke-interface {v5, v4}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 364
    .end local v11    # "item$iv$iv$iv":I
    move/from16 v2, v17

    move-object/from16 v6, v18

    move/from16 v4, v19

    goto :goto_6

    .line 375
    .end local v17    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    .restart local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_d
    move/from16 v17, v2

    move/from16 v19, v4

    move-object/from16 v18, v6

    .end local v2    # "numArgs$iv":I
    .end local v4    # "$i$f$pathNodesFromArgs":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v9    # "$i$f$mapTo":I
    .restart local v17    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 363
    nop

    .line 362
    .end local v7    # "$i$f$map":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v17    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 165
    :cond_e
    const/16 v2, 0x4c

    if-ne v0, v2, :cond_12

    const/4 v2, 0x2

    .restart local v2    # "numArgs$iv":I
    const/4 v4, 0x0

    .line 376
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    new-instance v6, Lkotlin/ranges/IntRange;

    array-length v7, v1

    sub-int/2addr v7, v2

    const/4 v8, 0x0

    invoke-direct {v6, v8, v7}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v6, Lkotlin/ranges/IntProgression;

    invoke-static {v6, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v6

    check-cast v6, Ljava/lang/Iterable;

    .restart local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v7, 0x0

    .line 377
    .restart local v7    # "$i$f$map":I
    new-instance v8, Ljava/util/ArrayList;

    invoke-static {v6, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v8, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v8

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v8, v6

    .restart local v8    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v9, 0x0

    .line 378
    .restart local v9    # "$i$f$mapTo":I
    invoke-interface {v8}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v10

    :goto_8
    invoke-interface {v10}, Ljava/util/Iterator;->hasNext()Z

    move-result v11

    if-eqz v11, :cond_11

    move-object v11, v10

    check-cast v11, Lkotlin/collections/IntIterator;

    invoke-virtual {v11}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v11

    .line 379
    .restart local v11    # "item$iv$iv$iv":I
    move v12, v11

    .restart local v12    # "index$iv":I
    const/4 v13, 0x0

    .line 380
    .restart local v13    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v14, v12, v2

    invoke-static {v12, v14}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v14

    invoke-static {v1, v14}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v14

    check-cast v14, Ljava/util/Collection;

    invoke-static {v14}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v14

    .line 381
    .restart local v14    # "subArray$iv":[F
    move-object v15, v14

    .restart local v15    # "array":[F
    const/16 v16, 0x0

    .line 166
    .local v16, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$4":I
    move/from16 v17, v2

    .end local v2    # "numArgs$iv":I
    .restart local v17    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    move/from16 v19, v4

    const/16 v18, 0x0

    .end local v4    # "$i$f$pathNodesFromArgs":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    aget v4, v15, v18

    move-object/from16 v18, v6

    .end local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v6, v15, v3

    invoke-direct {v2, v4, v6}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 382
    .end local v15    # "array":[F
    .end local v16    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$4":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 385
    instance-of v4, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v4, :cond_f

    if-lez v12, :cond_f

    new-instance v4, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v6, 0x0

    aget v15, v14, v6

    aget v6, v14, v3

    invoke-direct {v4, v15, v6}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_9

    .line 386
    :cond_f
    instance-of v4, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v4, :cond_10

    if-lez v12, :cond_10

    .line 387
    new-instance v4, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v6, 0x0

    aget v15, v14, v6

    aget v6, v14, v3

    invoke-direct {v4, v15, v6}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_9

    .line 388
    :cond_10
    move-object v4, v2

    .line 382
    :goto_9
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v12    # "index$iv":I
    .end local v13    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v14    # "subArray$iv":[F
    invoke-interface {v5, v4}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 378
    .end local v11    # "item$iv$iv$iv":I
    move/from16 v2, v17

    move-object/from16 v6, v18

    move/from16 v4, v19

    goto :goto_8

    .line 389
    .end local v17    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    .restart local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_11
    move/from16 v17, v2

    move/from16 v19, v4

    move-object/from16 v18, v6

    .end local v2    # "numArgs$iv":I
    .end local v4    # "$i$f$pathNodesFromArgs":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v9    # "$i$f$mapTo":I
    .restart local v17    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 377
    nop

    .line 376
    .end local v7    # "$i$f$map":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v17    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 169
    :cond_12
    const/16 v2, 0x68

    if-ne v0, v2, :cond_16

    const/4 v2, 0x1

    .restart local v2    # "numArgs$iv":I
    const/4 v4, 0x0

    .line 390
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    new-instance v6, Lkotlin/ranges/IntRange;

    array-length v7, v1

    sub-int/2addr v7, v2

    const/4 v8, 0x0

    invoke-direct {v6, v8, v7}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v6, Lkotlin/ranges/IntProgression;

    invoke-static {v6, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v6

    check-cast v6, Ljava/lang/Iterable;

    .restart local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v7, 0x0

    .line 391
    .restart local v7    # "$i$f$map":I
    new-instance v8, Ljava/util/ArrayList;

    invoke-static {v6, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v8, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v8

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v8, v6

    .restart local v8    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v9, 0x0

    .line 392
    .restart local v9    # "$i$f$mapTo":I
    invoke-interface {v8}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v10

    :goto_a
    invoke-interface {v10}, Ljava/util/Iterator;->hasNext()Z

    move-result v11

    if-eqz v11, :cond_15

    move-object v11, v10

    check-cast v11, Lkotlin/collections/IntIterator;

    invoke-virtual {v11}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v11

    .line 393
    .restart local v11    # "item$iv$iv$iv":I
    move v12, v11

    .restart local v12    # "index$iv":I
    const/4 v13, 0x0

    .line 394
    .restart local v13    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v14, v12, v2

    invoke-static {v12, v14}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v14

    invoke-static {v1, v14}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v14

    check-cast v14, Ljava/util/Collection;

    invoke-static {v14}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v14

    .line 395
    .restart local v14    # "subArray$iv":[F
    move-object v15, v14

    .restart local v15    # "array":[F
    const/16 v16, 0x0

    .line 170
    .local v16, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$5":I
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeHorizontalTo;

    move/from16 v19, v2

    const/16 v18, 0x0

    .end local v2    # "numArgs$iv":I
    .local v19, "numArgs$iv":I
    aget v2, v15, v18

    invoke-direct {v3, v2}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeHorizontalTo;-><init>(F)V

    move-object v2, v3

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 396
    .end local v15    # "array":[F
    .end local v16    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$5":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 399
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_13

    if-lez v12, :cond_13

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    aget v15, v14, v18

    move/from16 v18, v4

    const/16 v16, 0x1

    .end local v4    # "$i$f$pathNodesFromArgs":I
    .local v18, "$i$f$pathNodesFromArgs":I
    aget v4, v14, v16

    invoke-direct {v3, v15, v4}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    move-object/from16 v16, v2

    goto :goto_b

    .end local v18    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    :cond_13
    move/from16 v18, v4

    .line 400
    .end local v4    # "$i$f$pathNodesFromArgs":I
    .restart local v18    # "$i$f$pathNodesFromArgs":I
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_14

    if-lez v12, :cond_14

    .line 401
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v15, v14, v4

    move-object/from16 v16, v2

    const/4 v4, 0x1

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .local v16, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    aget v2, v14, v4

    invoke-direct {v3, v15, v2}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_b

    .line 400
    .end local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    :cond_14
    move-object/from16 v16, v2

    .line 402
    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    move-object/from16 v3, v16

    .line 396
    :goto_b
    nop

    .end local v12    # "index$iv":I
    .end local v13    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v14    # "subArray$iv":[F
    .end local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 392
    .end local v11    # "item$iv$iv$iv":I
    move/from16 v4, v18

    move/from16 v2, v19

    const/4 v3, 0x1

    goto :goto_a

    .line 403
    .end local v18    # "$i$f$pathNodesFromArgs":I
    .end local v19    # "numArgs$iv":I
    .local v2, "numArgs$iv":I
    .restart local v4    # "$i$f$pathNodesFromArgs":I
    :cond_15
    move/from16 v19, v2

    move/from16 v18, v4

    .end local v2    # "numArgs$iv":I
    .end local v4    # "$i$f$pathNodesFromArgs":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v8    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v9    # "$i$f$mapTo":I
    .restart local v18    # "$i$f$pathNodesFromArgs":I
    .restart local v19    # "numArgs$iv":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 391
    nop

    .line 390
    .end local v6    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v7    # "$i$f$map":I
    nop

    .end local v18    # "$i$f$pathNodesFromArgs":I
    .end local v19    # "numArgs$iv":I
    goto/16 :goto_2a

    .line 173
    :cond_16
    const/16 v2, 0x48

    if-ne v0, v2, :cond_1a

    const/4 v2, 0x1

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 404
    .local v3, "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .local v4, "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 405
    .local v6, "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .local v7, "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 406
    .local v8, "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_c
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_19

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 407
    .local v10, "item$iv$iv$iv":I
    move v11, v10

    .local v11, "index$iv":I
    const/4 v12, 0x0

    .line 408
    .local v12, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 409
    .local v13, "subArray$iv":[F
    move-object v14, v13

    .local v14, "array":[F
    const/4 v15, 0x0

    .line 174
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$6":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .local v16, "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$HorizontalTo;

    move/from16 v19, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .local v19, "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    invoke-direct {v2, v3}, Landroidx/compose/ui/graphics/vector/PathNode$HorizontalTo;-><init>(F)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 410
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$6":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 413
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_17

    if-lez v11, :cond_17

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    aget v14, v13, v18

    move-object/from16 v18, v4

    const/4 v15, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .local v18, "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v13, v15

    invoke-direct {v3, v14, v4}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_d

    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_17
    move-object/from16 v18, v4

    .line 414
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_18

    if-lez v11, :cond_18

    .line 415
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v14, v13, v4

    const/4 v4, 0x1

    aget v15, v13, v4

    invoke-direct {v3, v14, v15}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_d

    .line 416
    :cond_18
    move-object v3, v2

    .line 410
    :goto_d
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 406
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v19

    goto :goto_c

    .line 417
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_19
    move/from16 v16, v2

    move/from16 v19, v3

    move-object/from16 v18, v4

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 405
    nop

    .line 404
    .end local v6    # "$i$f$map":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v16    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 177
    :cond_1a
    const/16 v2, 0x76

    if-ne v0, v2, :cond_1e

    const/4 v2, 0x1

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 418
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 419
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 420
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_e
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_1d

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 421
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 422
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 423
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .restart local v14    # "array":[F
    const/4 v15, 0x0

    .line 178
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$7":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .restart local v16    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeVerticalTo;

    move/from16 v19, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    invoke-direct {v2, v3}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeVerticalTo;-><init>(F)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 424
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$7":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 427
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_1b

    if-lez v11, :cond_1b

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    aget v14, v13, v18

    move-object/from16 v18, v4

    const/4 v15, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v13, v15

    invoke-direct {v3, v14, v4}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_f

    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_1b
    move-object/from16 v18, v4

    .line 428
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_1c

    if-lez v11, :cond_1c

    .line 429
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v14, v13, v4

    const/4 v4, 0x1

    aget v15, v13, v4

    invoke-direct {v3, v14, v15}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_f

    .line 430
    :cond_1c
    move-object v3, v2

    .line 424
    :goto_f
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 420
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v19

    goto :goto_e

    .line 431
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_1d
    move/from16 v16, v2

    move/from16 v19, v3

    move-object/from16 v18, v4

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 419
    nop

    .line 418
    .end local v6    # "$i$f$map":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v16    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 181
    :cond_1e
    const/16 v2, 0x56

    if-ne v0, v2, :cond_22

    const/4 v2, 0x1

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 432
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 433
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 434
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_10
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_21

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 435
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 436
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 437
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .restart local v14    # "array":[F
    const/4 v15, 0x0

    .line 182
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$8":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .restart local v16    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$VerticalTo;

    move/from16 v19, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    invoke-direct {v2, v3}, Landroidx/compose/ui/graphics/vector/PathNode$VerticalTo;-><init>(F)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 438
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$8":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 441
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_1f

    if-lez v11, :cond_1f

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    aget v14, v13, v18

    move-object/from16 v18, v4

    const/4 v15, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v13, v15

    invoke-direct {v3, v14, v4}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_11

    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_1f
    move-object/from16 v18, v4

    .line 442
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_20

    if-lez v11, :cond_20

    .line 443
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v14, v13, v4

    const/4 v4, 0x1

    aget v15, v13, v4

    invoke-direct {v3, v14, v15}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_11

    .line 444
    :cond_20
    move-object v3, v2

    .line 438
    :goto_11
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 434
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v19

    goto :goto_10

    .line 445
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_21
    move/from16 v16, v2

    move/from16 v19, v3

    move-object/from16 v18, v4

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 433
    nop

    .line 432
    .end local v6    # "$i$f$map":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v16    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 185
    :cond_22
    const/16 v2, 0x63

    const/4 v4, 0x4

    const/4 v6, 0x3

    const/4 v7, 0x2

    if-ne v0, v2, :cond_26

    const/4 v2, 0x6

    .restart local v2    # "numArgs$iv":I
    const/4 v8, 0x0

    .line 446
    .local v8, "$i$f$pathNodesFromArgs":I
    new-instance v9, Lkotlin/ranges/IntRange;

    array-length v10, v1

    sub-int/2addr v10, v2

    const/4 v11, 0x0

    invoke-direct {v9, v11, v10}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v9, Lkotlin/ranges/IntProgression;

    invoke-static {v9, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v9

    check-cast v9, Ljava/lang/Iterable;

    .local v9, "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v10, 0x0

    .line 447
    .local v10, "$i$f$map":I
    new-instance v11, Ljava/util/ArrayList;

    invoke-static {v9, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v11, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v11

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v11, v9

    .local v11, "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v12, 0x0

    .line 448
    .local v12, "$i$f$mapTo":I
    invoke-interface {v11}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v13

    :goto_12
    invoke-interface {v13}, Ljava/util/Iterator;->hasNext()Z

    move-result v14

    if-eqz v14, :cond_25

    move-object v14, v13

    check-cast v14, Lkotlin/collections/IntIterator;

    invoke-virtual {v14}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v14

    .line 449
    .local v14, "item$iv$iv$iv":I
    move v15, v14

    .local v15, "index$iv":I
    const/16 v16, 0x0

    .line 450
    .local v16, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v3, v15, v2

    invoke-static {v15, v3}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v3

    invoke-static {v1, v3}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v3

    check-cast v3, Ljava/util/Collection;

    invoke-static {v3}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v3

    .line 451
    .local v3, "subArray$iv":[F
    move-object/from16 v19, v3

    .local v19, "array":[F
    const/16 v21, 0x0

    .line 186
    .local v21, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$9":I
    new-instance v29, Landroidx/compose/ui/graphics/vector/PathNode$RelativeCurveTo;

    .line 187
    const/16 v20, 0x0

    aget v23, v19, v20

    .line 188
    const/16 v17, 0x1

    aget v24, v19, v17

    .line 189
    aget v25, v19, v7

    .line 190
    aget v26, v19, v6

    .line 191
    aget v27, v19, v4

    .line 192
    const/16 v18, 0x5

    aget v28, v19, v18

    .line 186
    move-object/from16 v22, v29

    invoke-direct/range {v22 .. v28}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeCurveTo;-><init>(FFFFFF)V

    move-object/from16 v4, v29

    check-cast v4, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 452
    .end local v19    # "array":[F
    .end local v21    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$9":I
    .local v4, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 455
    instance-of v6, v4, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v6, :cond_23

    if-lez v15, :cond_23

    new-instance v6, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/16 v20, 0x0

    aget v7, v3, v20

    move/from16 v23, v2

    const/16 v17, 0x1

    .end local v2    # "numArgs$iv":I
    .local v23, "numArgs$iv":I
    aget v2, v3, v17

    invoke-direct {v6, v7, v2}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v6, Landroidx/compose/ui/graphics/vector/PathNode;

    move-object/from16 v24, v4

    goto :goto_13

    .end local v23    # "numArgs$iv":I
    .restart local v2    # "numArgs$iv":I
    :cond_23
    move/from16 v23, v2

    .line 456
    .end local v2    # "numArgs$iv":I
    .restart local v23    # "numArgs$iv":I
    instance-of v2, v4, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v2, :cond_24

    if-lez v15, :cond_24

    .line 457
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v6, 0x0

    aget v7, v3, v6

    move-object/from16 v24, v4

    const/4 v6, 0x1

    .end local v4    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .local v24, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    aget v4, v3, v6

    invoke-direct {v2, v7, v4}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    move-object v6, v2

    check-cast v6, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_13

    .line 456
    .end local v24    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v4    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    :cond_24
    move-object/from16 v24, v4

    .line 458
    .end local v4    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v24    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    move-object/from16 v6, v24

    .line 452
    :goto_13
    nop

    .end local v3    # "subArray$iv":[F
    .end local v15    # "index$iv":I
    .end local v16    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v24    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    invoke-interface {v5, v6}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 448
    .end local v14    # "item$iv$iv$iv":I
    move/from16 v2, v23

    const/4 v4, 0x4

    const/4 v6, 0x3

    const/4 v7, 0x2

    goto/16 :goto_12

    .line 459
    .end local v23    # "numArgs$iv":I
    .restart local v2    # "numArgs$iv":I
    :cond_25
    move/from16 v23, v2

    .end local v2    # "numArgs$iv":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v11    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v12    # "$i$f$mapTo":I
    .restart local v23    # "numArgs$iv":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 447
    nop

    .line 446
    .end local v9    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v10    # "$i$f$map":I
    nop

    .end local v8    # "$i$f$pathNodesFromArgs":I
    .end local v23    # "numArgs$iv":I
    goto/16 :goto_2a

    .line 196
    :cond_26
    const/16 v2, 0x43

    if-ne v0, v2, :cond_2a

    const/4 v2, 0x6

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 460
    .local v3, "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .local v4, "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 461
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 462
    .local v8, "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_14
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_29

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 463
    .local v10, "item$iv$iv$iv":I
    move v11, v10

    .local v11, "index$iv":I
    const/4 v12, 0x0

    .line 464
    .local v12, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 465
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .local v14, "array":[F
    const/4 v15, 0x0

    .line 197
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$10":I
    new-instance v16, Landroidx/compose/ui/graphics/vector/PathNode$CurveTo;

    .line 198
    const/16 v20, 0x0

    aget v24, v14, v20

    .line 199
    const/16 v17, 0x1

    aget v25, v14, v17

    .line 200
    const/16 v21, 0x2

    aget v26, v14, v21

    .line 201
    const/16 v19, 0x3

    aget v27, v14, v19

    .line 202
    const/16 v22, 0x4

    aget v28, v14, v22

    .line 203
    const/16 v18, 0x5

    aget v29, v14, v18

    .line 197
    move-object/from16 v23, v16

    invoke-direct/range {v23 .. v29}, Landroidx/compose/ui/graphics/vector/PathNode$CurveTo;-><init>(FFFFFF)V

    move-object/from16 v14, v16

    check-cast v14, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 466
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$10":I
    .local v14, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 469
    instance-of v15, v14, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v15, :cond_27

    if-lez v11, :cond_27

    new-instance v15, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    move/from16 v23, v2

    const/16 v16, 0x0

    .end local v2    # "numArgs$iv":I
    .restart local v23    # "numArgs$iv":I
    aget v2, v13, v16

    move/from16 v24, v3

    const/16 v16, 0x1

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .local v24, "$i$f$pathNodesFromArgs":I
    aget v3, v13, v16

    invoke-direct {v15, v2, v3}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v15, Landroidx/compose/ui/graphics/vector/PathNode;

    move-object/from16 v16, v4

    goto :goto_15

    .end local v23    # "numArgs$iv":I
    .end local v24    # "$i$f$pathNodesFromArgs":I
    .restart local v2    # "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    :cond_27
    move/from16 v23, v2

    move/from16 v24, v3

    .line 470
    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v23    # "numArgs$iv":I
    .restart local v24    # "$i$f$pathNodesFromArgs":I
    instance-of v2, v14, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v2, :cond_28

    if-lez v11, :cond_28

    .line 471
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v3, 0x0

    aget v15, v13, v3

    move-object/from16 v16, v4

    const/4 v3, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .local v16, "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v13, v3

    invoke-direct {v2, v15, v4}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    move-object v15, v2

    check-cast v15, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_15

    .line 470
    .end local v16    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_28
    move-object/from16 v16, v4

    .line 472
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v16    # "$this$map$iv$iv":Ljava/lang/Iterable;
    move-object v15, v14

    .line 466
    :goto_15
    nop

    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    .end local v14    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    invoke-interface {v5, v15}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 462
    .end local v10    # "item$iv$iv$iv":I
    move-object/from16 v4, v16

    move/from16 v2, v23

    move/from16 v3, v24

    goto/16 :goto_14

    .line 473
    .end local v16    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v23    # "numArgs$iv":I
    .end local v24    # "$i$f$pathNodesFromArgs":I
    .restart local v2    # "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_29
    move/from16 v23, v2

    move/from16 v24, v3

    move-object/from16 v16, v4

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v23    # "numArgs$iv":I
    .restart local v24    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 461
    nop

    .line 460
    .end local v6    # "$i$f$map":I
    .end local v16    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v23    # "numArgs$iv":I
    .end local v24    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 207
    :cond_2a
    const/16 v2, 0x73

    if-ne v0, v2, :cond_2e

    const/4 v2, 0x4

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 474
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 475
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 476
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_16
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_2d

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 477
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 478
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 479
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .local v14, "array":[F
    const/4 v15, 0x0

    .line 208
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$11":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .local v16, "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeReflectiveCurveTo;

    .line 209
    move/from16 v22, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .local v22, "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    .line 210
    move-object/from16 v18, v4

    const/16 v17, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v14, v17

    .line 211
    move/from16 v23, v6

    const/16 v21, 0x2

    .end local v6    # "$i$f$map":I
    .local v23, "$i$f$map":I
    aget v6, v14, v21

    .line 212
    move-object/from16 v24, v7

    const/16 v19, 0x3

    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .local v24, "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    aget v7, v14, v19

    .line 208
    invoke-direct {v2, v3, v4, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeReflectiveCurveTo;-><init>(FFFF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 480
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$11":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 483
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_2b

    if-lez v11, :cond_2b

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_17

    .line 484
    :cond_2b
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_2c

    if-lez v11, :cond_2c

    .line 485
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_17

    .line 486
    :cond_2c
    move-object v3, v2

    .line 480
    :goto_17
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 476
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v22

    move/from16 v6, v23

    move-object/from16 v7, v24

    goto :goto_16

    .line 487
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v22    # "$i$f$pathNodesFromArgs":I
    .end local v23    # "$i$f$map":I
    .end local v24    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v6    # "$i$f$map":I
    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    :cond_2d
    move/from16 v16, v2

    move/from16 v22, v3

    move-object/from16 v18, v4

    move/from16 v23, v6

    move-object/from16 v24, v7

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$i$f$map":I
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v22    # "$i$f$pathNodesFromArgs":I
    .restart local v23    # "$i$f$map":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 475
    nop

    .line 474
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v23    # "$i$f$map":I
    nop

    .end local v16    # "numArgs$iv":I
    .end local v22    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 216
    :cond_2e
    const/16 v2, 0x53

    if-ne v0, v2, :cond_32

    const/4 v2, 0x4

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 488
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 489
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 490
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_18
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_31

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 491
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 492
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 493
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .restart local v14    # "array":[F
    const/4 v15, 0x0

    .line 217
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$12":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .restart local v16    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$ReflectiveCurveTo;

    .line 218
    move/from16 v22, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v22    # "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    .line 219
    move-object/from16 v18, v4

    const/16 v17, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v14, v17

    .line 220
    move/from16 v23, v6

    const/16 v21, 0x2

    .end local v6    # "$i$f$map":I
    .restart local v23    # "$i$f$map":I
    aget v6, v14, v21

    .line 221
    move-object/from16 v24, v7

    const/16 v19, 0x3

    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .restart local v24    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    aget v7, v14, v19

    .line 217
    invoke-direct {v2, v3, v4, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$ReflectiveCurveTo;-><init>(FFFF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 494
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$12":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 497
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_2f

    if-lez v11, :cond_2f

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_19

    .line 498
    :cond_2f
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_30

    if-lez v11, :cond_30

    .line 499
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_19

    .line 500
    :cond_30
    move-object v3, v2

    .line 494
    :goto_19
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 490
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v22

    move/from16 v6, v23

    move-object/from16 v7, v24

    goto :goto_18

    .line 501
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v22    # "$i$f$pathNodesFromArgs":I
    .end local v23    # "$i$f$map":I
    .end local v24    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v6    # "$i$f$map":I
    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    :cond_31
    move/from16 v16, v2

    move/from16 v22, v3

    move-object/from16 v18, v4

    move/from16 v23, v6

    move-object/from16 v24, v7

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$i$f$map":I
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v22    # "$i$f$pathNodesFromArgs":I
    .restart local v23    # "$i$f$map":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 489
    nop

    .line 488
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v23    # "$i$f$map":I
    nop

    .end local v16    # "numArgs$iv":I
    .end local v22    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 225
    :cond_32
    const/16 v2, 0x71

    if-ne v0, v2, :cond_36

    const/4 v2, 0x4

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 502
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 503
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 504
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_1a
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_35

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 505
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 506
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 507
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .restart local v14    # "array":[F
    const/4 v15, 0x0

    .line 226
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$13":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .restart local v16    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeQuadTo;

    .line 227
    move/from16 v22, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v22    # "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    .line 228
    move-object/from16 v18, v4

    const/16 v17, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v14, v17

    .line 229
    move/from16 v23, v6

    const/16 v21, 0x2

    .end local v6    # "$i$f$map":I
    .restart local v23    # "$i$f$map":I
    aget v6, v14, v21

    .line 230
    move-object/from16 v24, v7

    const/16 v19, 0x3

    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .restart local v24    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    aget v7, v14, v19

    .line 226
    invoke-direct {v2, v3, v4, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeQuadTo;-><init>(FFFF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 508
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$13":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 511
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_33

    if-lez v11, :cond_33

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1b

    .line 512
    :cond_33
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_34

    if-lez v11, :cond_34

    .line 513
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1b

    .line 514
    :cond_34
    move-object v3, v2

    .line 508
    :goto_1b
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 504
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v22

    move/from16 v6, v23

    move-object/from16 v7, v24

    goto :goto_1a

    .line 515
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v22    # "$i$f$pathNodesFromArgs":I
    .end local v23    # "$i$f$map":I
    .end local v24    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v6    # "$i$f$map":I
    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    :cond_35
    move/from16 v16, v2

    move/from16 v22, v3

    move-object/from16 v18, v4

    move/from16 v23, v6

    move-object/from16 v24, v7

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$i$f$map":I
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v22    # "$i$f$pathNodesFromArgs":I
    .restart local v23    # "$i$f$map":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 503
    nop

    .line 502
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v23    # "$i$f$map":I
    nop

    .end local v16    # "numArgs$iv":I
    .end local v22    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 234
    :cond_36
    const/16 v2, 0x51

    if-ne v0, v2, :cond_3a

    const/4 v2, 0x4

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 516
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 517
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 518
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_1c
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_39

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 519
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 520
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 521
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .restart local v14    # "array":[F
    const/4 v15, 0x0

    .line 235
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$14":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .restart local v16    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$QuadTo;

    .line 236
    move/from16 v22, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v22    # "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    .line 237
    move-object/from16 v18, v4

    const/16 v17, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v14, v17

    .line 238
    move/from16 v23, v6

    const/16 v21, 0x2

    .end local v6    # "$i$f$map":I
    .restart local v23    # "$i$f$map":I
    aget v6, v14, v21

    .line 239
    move-object/from16 v24, v7

    const/16 v19, 0x3

    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .restart local v24    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    aget v7, v14, v19

    .line 235
    invoke-direct {v2, v3, v4, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$QuadTo;-><init>(FFFF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 522
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$14":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 525
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_37

    if-lez v11, :cond_37

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1d

    .line 526
    :cond_37
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_38

    if-lez v11, :cond_38

    .line 527
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v6, v13, v4

    const/4 v4, 0x1

    aget v7, v13, v4

    invoke-direct {v3, v6, v7}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1d

    .line 528
    :cond_38
    move-object v3, v2

    .line 522
    :goto_1d
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 518
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v22

    move/from16 v6, v23

    move-object/from16 v7, v24

    goto :goto_1c

    .line 529
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v22    # "$i$f$pathNodesFromArgs":I
    .end local v23    # "$i$f$map":I
    .end local v24    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v6    # "$i$f$map":I
    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    :cond_39
    move/from16 v16, v2

    move/from16 v22, v3

    move-object/from16 v18, v4

    move/from16 v23, v6

    move-object/from16 v24, v7

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$i$f$map":I
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v22    # "$i$f$pathNodesFromArgs":I
    .restart local v23    # "$i$f$map":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 517
    nop

    .line 516
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v23    # "$i$f$map":I
    nop

    .end local v16    # "numArgs$iv":I
    .end local v22    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 243
    :cond_3a
    const/16 v2, 0x74

    if-ne v0, v2, :cond_3e

    const/4 v2, 0x2

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 530
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 531
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 532
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_1e
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_3d

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 533
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 534
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 535
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .restart local v14    # "array":[F
    const/4 v15, 0x0

    .line 244
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$15":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .restart local v16    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeReflectiveQuadTo;

    move/from16 v19, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .local v19, "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    move-object/from16 v18, v4

    const/16 v17, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v14, v17

    invoke-direct {v2, v3, v4}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeReflectiveQuadTo;-><init>(FF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 536
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$15":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 539
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_3b

    if-lez v11, :cond_3b

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v4, 0x0

    aget v14, v13, v4

    const/4 v4, 0x1

    aget v15, v13, v4

    invoke-direct {v3, v14, v15}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1f

    .line 540
    :cond_3b
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_3c

    if-lez v11, :cond_3c

    .line 541
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v14, v13, v4

    const/4 v4, 0x1

    aget v15, v13, v4

    invoke-direct {v3, v14, v15}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_1f

    .line 542
    :cond_3c
    move-object v3, v2

    .line 536
    :goto_1f
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 532
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v19

    goto :goto_1e

    .line 543
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_3d
    move/from16 v16, v2

    move/from16 v19, v3

    move-object/from16 v18, v4

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 531
    nop

    .line 530
    .end local v6    # "$i$f$map":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v16    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 247
    :cond_3e
    const/16 v2, 0x54

    if-ne v0, v2, :cond_42

    const/4 v2, 0x2

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 544
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 545
    .restart local v6    # "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .restart local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 546
    .restart local v8    # "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_20
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_41

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 547
    .restart local v10    # "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 548
    .restart local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 549
    .restart local v13    # "subArray$iv":[F
    move-object v14, v13

    .restart local v14    # "array":[F
    const/4 v15, 0x0

    .line 248
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$16":I
    move/from16 v16, v2

    .end local v2    # "numArgs$iv":I
    .restart local v16    # "numArgs$iv":I
    new-instance v2, Landroidx/compose/ui/graphics/vector/PathNode$ReflectiveQuadTo;

    move/from16 v19, v3

    const/16 v18, 0x0

    .end local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    aget v3, v14, v18

    move-object/from16 v18, v4

    const/16 v17, 0x1

    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    aget v4, v14, v17

    invoke-direct {v2, v3, v4}, Landroidx/compose/ui/graphics/vector/PathNode$ReflectiveQuadTo;-><init>(FF)V

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 550
    .end local v14    # "array":[F
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$16":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 553
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_3f

    if-lez v11, :cond_3f

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/4 v4, 0x0

    aget v14, v13, v4

    const/4 v4, 0x1

    aget v15, v13, v4

    invoke-direct {v3, v14, v15}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_21

    .line 554
    :cond_3f
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_40

    if-lez v11, :cond_40

    .line 555
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v14, v13, v4

    const/4 v4, 0x1

    aget v15, v13, v4

    invoke-direct {v3, v14, v15}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_21

    .line 556
    :cond_40
    move-object v3, v2

    .line 550
    :goto_21
    nop

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 546
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v2, v16

    move-object/from16 v4, v18

    move/from16 v3, v19

    goto :goto_20

    .line 557
    .end local v16    # "numArgs$iv":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v19    # "$i$f$pathNodesFromArgs":I
    .local v2, "numArgs$iv":I
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    :cond_41
    move/from16 v16, v2

    move/from16 v19, v3

    move-object/from16 v18, v4

    .end local v2    # "numArgs$iv":I
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v16    # "numArgs$iv":I
    .restart local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .restart local v19    # "$i$f$pathNodesFromArgs":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 545
    nop

    .line 544
    .end local v6    # "$i$f$map":I
    .end local v18    # "$this$map$iv$iv":Ljava/lang/Iterable;
    nop

    .end local v16    # "numArgs$iv":I
    .end local v19    # "$i$f$pathNodesFromArgs":I
    goto/16 :goto_2a

    .line 251
    :cond_42
    const/16 v2, 0x61

    const/4 v4, 0x0

    if-ne v0, v2, :cond_48

    const/4 v2, 0x7

    .restart local v2    # "numArgs$iv":I
    const/4 v6, 0x0

    .line 558
    .local v6, "$i$f$pathNodesFromArgs":I
    new-instance v7, Lkotlin/ranges/IntRange;

    array-length v8, v1

    sub-int/2addr v8, v2

    const/4 v9, 0x0

    invoke-direct {v7, v9, v8}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v7, Lkotlin/ranges/IntProgression;

    invoke-static {v7, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v7

    check-cast v7, Ljava/lang/Iterable;

    .local v7, "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 559
    .local v8, "$i$f$map":I
    new-instance v9, Ljava/util/ArrayList;

    invoke-static {v7, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v9, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v9

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v9, v7

    .local v9, "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v10, 0x0

    .line 560
    .local v10, "$i$f$mapTo":I
    invoke-interface {v9}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v11

    :goto_22
    invoke-interface {v11}, Ljava/util/Iterator;->hasNext()Z

    move-result v12

    if-eqz v12, :cond_47

    move-object v12, v11

    check-cast v12, Lkotlin/collections/IntIterator;

    invoke-virtual {v12}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v12

    .line 561
    .local v12, "item$iv$iv$iv":I
    move v13, v12

    .local v13, "index$iv":I
    const/4 v14, 0x0

    .line 562
    .local v14, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v15, v13, v2

    invoke-static {v13, v15}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v15

    invoke-static {v1, v15}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v15

    check-cast v15, Ljava/util/Collection;

    invoke-static {v15}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v15

    .line 563
    .local v15, "subArray$iv":[F
    move-object/from16 v16, v15

    .local v16, "array":[F
    const/16 v23, 0x0

    .line 252
    .local v23, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$17":I
    new-instance v32, Landroidx/compose/ui/graphics/vector/PathNode$RelativeArcTo;

    .line 253
    const/16 v20, 0x0

    aget v25, v16, v20

    .line 254
    const/16 v17, 0x1

    aget v26, v16, v17

    .line 255
    const/16 v21, 0x2

    aget v27, v16, v21

    .line 256
    const/16 v19, 0x3

    aget v3, v16, v19

    invoke-static {v3, v4}, Ljava/lang/Float;->compare(FF)I

    move-result v3

    if-eqz v3, :cond_43

    const/16 v28, 0x1

    goto :goto_23

    :cond_43
    const/16 v28, 0x0

    .line 257
    :goto_23
    move/from16 v33, v2

    const/4 v3, 0x4

    .end local v2    # "numArgs$iv":I
    .local v33, "numArgs$iv":I
    aget v2, v16, v3

    invoke-static {v2, v4}, Ljava/lang/Float;->compare(FF)I

    move-result v2

    if-eqz v2, :cond_44

    const/16 v29, 0x1

    goto :goto_24

    :cond_44
    const/16 v29, 0x0

    .line 258
    :goto_24
    const/4 v2, 0x5

    aget v30, v16, v2

    .line 259
    const/4 v2, 0x6

    aget v31, v16, v2

    .line 252
    move-object/from16 v24, v32

    invoke-direct/range {v24 .. v31}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeArcTo;-><init>(FFFZZFF)V

    move-object/from16 v2, v32

    check-cast v2, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 564
    .end local v16    # "array":[F
    .end local v23    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$17":I
    .local v2, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 567
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v3, :cond_45

    if-lez v13, :cond_45

    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/16 v16, 0x0

    aget v4, v15, v16

    move/from16 v24, v6

    const/16 v16, 0x1

    .end local v6    # "$i$f$pathNodesFromArgs":I
    .local v24, "$i$f$pathNodesFromArgs":I
    aget v6, v15, v16

    invoke-direct {v3, v4, v6}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    move-object/from16 v16, v2

    goto :goto_25

    .end local v24    # "$i$f$pathNodesFromArgs":I
    .restart local v6    # "$i$f$pathNodesFromArgs":I
    :cond_45
    move/from16 v24, v6

    .line 568
    .end local v6    # "$i$f$pathNodesFromArgs":I
    .restart local v24    # "$i$f$pathNodesFromArgs":I
    instance-of v3, v2, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v3, :cond_46

    if-lez v13, :cond_46

    .line 569
    new-instance v3, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v4, 0x0

    aget v6, v15, v4

    move-object/from16 v16, v2

    const/4 v4, 0x1

    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .local v16, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    aget v2, v15, v4

    invoke-direct {v3, v6, v2}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    check-cast v3, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_25

    .line 568
    .end local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    :cond_46
    move-object/from16 v16, v2

    .line 570
    .end local v2    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    .restart local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    move-object/from16 v3, v16

    .line 564
    :goto_25
    nop

    .end local v13    # "index$iv":I
    .end local v14    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v15    # "subArray$iv":[F
    .end local v16    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    invoke-interface {v5, v3}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 560
    .end local v12    # "item$iv$iv$iv":I
    move/from16 v6, v24

    move/from16 v2, v33

    const/4 v4, 0x0

    goto/16 :goto_22

    .line 571
    .end local v24    # "$i$f$pathNodesFromArgs":I
    .end local v33    # "numArgs$iv":I
    .local v2, "numArgs$iv":I
    .restart local v6    # "$i$f$pathNodesFromArgs":I
    :cond_47
    move/from16 v33, v2

    move/from16 v24, v6

    .end local v2    # "numArgs$iv":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v6    # "$i$f$pathNodesFromArgs":I
    .end local v9    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v10    # "$i$f$mapTo":I
    .restart local v24    # "$i$f$pathNodesFromArgs":I
    .restart local v33    # "numArgs$iv":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 559
    nop

    .line 558
    .end local v7    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$map":I
    nop

    .end local v24    # "$i$f$pathNodesFromArgs":I
    .end local v33    # "numArgs$iv":I
    goto/16 :goto_2a

    .line 263
    :cond_48
    const/16 v2, 0x41

    if-ne v0, v2, :cond_4e

    const/4 v2, 0x7

    .restart local v2    # "numArgs$iv":I
    const/4 v3, 0x0

    .line 572
    .restart local v3    # "$i$f$pathNodesFromArgs":I
    new-instance v4, Lkotlin/ranges/IntRange;

    array-length v6, v1

    sub-int/2addr v6, v2

    const/4 v7, 0x0

    invoke-direct {v4, v7, v6}, Lkotlin/ranges/IntRange;-><init>(II)V

    check-cast v4, Lkotlin/ranges/IntProgression;

    invoke-static {v4, v2}, Lkotlin/ranges/RangesKt;->step(Lkotlin/ranges/IntProgression;I)Lkotlin/ranges/IntProgression;

    move-result-object v4

    check-cast v4, Ljava/lang/Iterable;

    .restart local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    const/4 v6, 0x0

    .line 573
    .local v6, "$i$f$map":I
    new-instance v7, Ljava/util/ArrayList;

    invoke-static {v4, v5}, Lkotlin/collections/CollectionsKt;->collectionSizeOrDefault(Ljava/lang/Iterable;I)I

    move-result v5

    invoke-direct {v7, v5}, Ljava/util/ArrayList;-><init>(I)V

    move-object v5, v7

    check-cast v5, Ljava/util/Collection;

    .restart local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    move-object v7, v4

    .local v7, "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    const/4 v8, 0x0

    .line 574
    .local v8, "$i$f$mapTo":I
    invoke-interface {v7}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v9

    :goto_26
    invoke-interface {v9}, Ljava/util/Iterator;->hasNext()Z

    move-result v10

    if-eqz v10, :cond_4d

    move-object v10, v9

    check-cast v10, Lkotlin/collections/IntIterator;

    invoke-virtual {v10}, Lkotlin/collections/IntIterator;->nextInt()I

    move-result v10

    .line 575
    .local v10, "item$iv$iv$iv":I
    move v11, v10

    .restart local v11    # "index$iv":I
    const/4 v12, 0x0

    .line 576
    .local v12, "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    add-int v13, v11, v2

    invoke-static {v11, v13}, Lkotlin/ranges/RangesKt;->until(II)Lkotlin/ranges/IntRange;

    move-result-object v13

    invoke-static {v1, v13}, Lkotlin/collections/ArraysKt;->slice([FLkotlin/ranges/IntRange;)Ljava/util/List;

    move-result-object v13

    check-cast v13, Ljava/util/Collection;

    invoke-static {v13}, Lkotlin/collections/CollectionsKt;->toFloatArray(Ljava/util/Collection;)[F

    move-result-object v13

    .line 577
    .local v13, "subArray$iv":[F
    move-object v14, v13

    .local v14, "array":[F
    const/4 v15, 0x0

    .line 264
    .local v15, "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$18":I
    new-instance v16, Landroidx/compose/ui/graphics/vector/PathNode$ArcTo;

    .line 265
    const/16 v20, 0x0

    aget v25, v14, v20

    .line 266
    const/16 v17, 0x1

    aget v26, v14, v17

    .line 267
    const/16 v21, 0x2

    aget v27, v14, v21

    .line 268
    const/16 v19, 0x3

    aget v0, v14, v19

    const/4 v1, 0x0

    invoke-static {v0, v1}, Ljava/lang/Float;->compare(FF)I

    move-result v0

    if-eqz v0, :cond_49

    const/16 v28, 0x1

    goto :goto_27

    :cond_49
    const/16 v28, 0x0

    .line 269
    :goto_27
    move/from16 v22, v2

    const/4 v0, 0x4

    .end local v2    # "numArgs$iv":I
    .local v22, "numArgs$iv":I
    aget v2, v14, v0

    invoke-static {v2, v1}, Ljava/lang/Float;->compare(FF)I

    move-result v2

    if-eqz v2, :cond_4a

    const/16 v29, 0x1

    goto :goto_28

    :cond_4a
    const/16 v29, 0x0

    .line 270
    :goto_28
    const/4 v2, 0x5

    aget v30, v14, v2

    .line 271
    const/16 v18, 0x6

    aget v31, v14, v18

    .line 264
    move-object/from16 v24, v16

    invoke-direct/range {v24 .. v31}, Landroidx/compose/ui/graphics/vector/PathNode$ArcTo;-><init>(FFFZZFF)V

    move-object/from16 v14, v16

    check-cast v14, Landroidx/compose/ui/graphics/vector/PathNode;

    .line 578
    .end local v15    # "$i$a$-pathNodesFromArgs-PathNodeKt$toPathNodes$18":I
    .local v14, "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    nop

    .line 581
    instance-of v15, v14, Landroidx/compose/ui/graphics/vector/PathNode$MoveTo;

    if-eqz v15, :cond_4b

    if-lez v11, :cond_4b

    new-instance v15, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;

    const/16 v16, 0x0

    aget v0, v13, v16

    const/16 v16, 0x1

    aget v1, v13, v16

    invoke-direct {v15, v0, v1}, Landroidx/compose/ui/graphics/vector/PathNode$LineTo;-><init>(FF)V

    check-cast v15, Landroidx/compose/ui/graphics/vector/PathNode;

    const/16 v16, 0x1

    goto :goto_29

    .line 582
    :cond_4b
    instance-of v0, v14, Landroidx/compose/ui/graphics/vector/PathNode$RelativeMoveTo;

    if-eqz v0, :cond_4c

    if-lez v11, :cond_4c

    .line 583
    new-instance v0, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;

    const/4 v1, 0x0

    aget v15, v13, v1

    const/16 v16, 0x1

    aget v1, v13, v16

    invoke-direct {v0, v15, v1}, Landroidx/compose/ui/graphics/vector/PathNode$RelativeLineTo;-><init>(FF)V

    move-object v15, v0

    check-cast v15, Landroidx/compose/ui/graphics/vector/PathNode;

    goto :goto_29

    .line 582
    :cond_4c
    const/16 v16, 0x1

    .line 584
    move-object v15, v14

    .line 578
    :goto_29
    nop

    .end local v11    # "index$iv":I
    .end local v12    # "$i$a$-map-PathNodeKt$pathNodesFromArgs$1$iv":I
    .end local v13    # "subArray$iv":[F
    .end local v14    # "node$iv":Landroidx/compose/ui/graphics/vector/PathNode;
    invoke-interface {v5, v15}, Ljava/util/Collection;->add(Ljava/lang/Object;)Z

    .line 574
    .end local v10    # "item$iv$iv$iv":I
    move/from16 v0, p0

    move-object/from16 v1, p1

    move/from16 v2, v22

    goto/16 :goto_26

    .line 585
    .end local v22    # "numArgs$iv":I
    .restart local v2    # "numArgs$iv":I
    :cond_4d
    move/from16 v22, v2

    .end local v2    # "numArgs$iv":I
    .end local v5    # "destination$iv$iv$iv":Ljava/util/Collection;
    .end local v7    # "$this$mapTo$iv$iv$iv":Ljava/lang/Iterable;
    .end local v8    # "$i$f$mapTo":I
    .restart local v22    # "numArgs$iv":I
    move-object v2, v5

    check-cast v2, Ljava/util/List;

    .line 573
    nop

    .line 572
    .end local v4    # "$this$map$iv$iv":Ljava/lang/Iterable;
    .end local v6    # "$i$f$map":I
    nop

    .line 276
    .end local v3    # "$i$f$pathNodesFromArgs":I
    .end local v22    # "numArgs$iv":I
    :goto_2a
    return-object v2

    .line 275
    :cond_4e
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Unknown command for: "

    invoke-static/range {p0 .. p0}, Ljava/lang/Character;->valueOf(C)Ljava/lang/Character;

    move-result-object v2

    invoke-static {v1, v2}, Lkotlin/jvm/internal/Intrinsics;->stringPlus(Ljava/lang/String;Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v1

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    throw v0
.end method
