.class public Landroidx/compose/runtime/snapshots/MutableSnapshot;
.super Landroidx/compose/runtime/snapshots/Snapshot;
.source "Snapshot.kt"


# annotations
.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nSnapshot.kt\nKotlin\n*S Kotlin\n*F\n+ 1 Snapshot.kt\nandroidx/compose/runtime/snapshots/MutableSnapshot\n+ 2 Snapshot.kt\nandroidx/compose/runtime/snapshots/SnapshotKt\n+ 3 ActualJvm.jvm.kt\nandroidx/compose/runtime/ActualJvm_jvmKt\n+ 4 ListUtils.kt\nandroidx/compose/runtime/snapshots/ListUtilsKt\n+ 5 fake.kt\nkotlin/jvm/internal/FakeKt\n*L\n1#1,1767:1\n814#1,2:1768\n816#1,2:1772\n818#1,9:1776\n814#1,2:1799\n816#1,2:1803\n818#1,9:1807\n814#1,4:1827\n818#1,9:1833\n1371#2:1770\n1371#2:1774\n1371#2:1785\n1371#2:1801\n1371#2:1805\n1371#2:1816\n1371#2:1822\n1371#2:1825\n1371#2:1831\n1371#2:1842\n1371#2:1844\n62#3:1771\n62#3:1775\n62#3:1786\n62#3:1802\n62#3:1806\n62#3:1817\n62#3:1823\n62#3:1826\n62#3:1832\n62#3:1843\n62#3:1845\n28#4,6:1787\n28#4,6:1793\n28#4,4:1818\n33#4:1824\n1#5:1846\n*S KotlinDebug\n*F\n+ 1 Snapshot.kt\nandroidx/compose/runtime/snapshots/MutableSnapshot\n*L\n535#1:1768,2\n535#1:1772,2\n535#1:1776,9\n651#1:1799,2\n651#1:1803,2\n651#1:1807,9\n829#1:1827,4\n829#1:1833,9\n536#1:1770\n535#1:1774\n584#1:1785\n652#1:1801\n651#1:1805\n686#1:1816\n798#1:1822\n817#1:1825\n829#1:1831\n832#1:1842\n838#1:1844\n536#1:1771\n535#1:1775\n584#1:1786\n652#1:1802\n651#1:1806\n686#1:1817\n798#1:1823\n817#1:1826\n829#1:1832\n832#1:1843\n838#1:1845\n622#1:1787,6\n628#1:1793,6\n795#1:1818,4\n795#1:1824\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u0000V\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0008\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\u0010\u0002\n\u0002\u0008\u0003\n\u0002\u0010\u000b\n\u0002\u0008\u0005\n\u0002\u0010#\n\u0002\u0018\u0002\n\u0002\u0008\u0018\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0007\n\u0002\u0010$\n\u0002\u0018\u0002\n\u0002\u0008\u0015\u0008\u0017\u0018\u00002\u00020\u0001BC\u0008\u0000\u0012\u0006\u0010\u0002\u001a\u00020\u0003\u0012\u0006\u0010\u0004\u001a\u00020\u0005\u0012\u0014\u0010\u0006\u001a\u0010\u0012\u0004\u0012\u00020\u0008\u0012\u0004\u0012\u00020\t\u0018\u00010\u0007\u0012\u0014\u0010\n\u001a\u0010\u0012\u0004\u0012\u00020\u0008\u0012\u0004\u0012\u00020\t\u0018\u00010\u0007\u00a2\u0006\u0002\u0010\u000bJ\u0008\u0010(\u001a\u00020\tH\u0002J\r\u0010)\u001a\u00020\tH\u0000\u00a2\u0006\u0002\u0008*J\'\u0010)\u001a\u0002H+\"\u0004\u0008\u0000\u0010+2\u000c\u0010,\u001a\u0008\u0012\u0004\u0012\u0002H+0-H\u0080\u0008\u00f8\u0001\u0000\u00a2\u0006\u0004\u0008*\u0010.J\u0008\u0010/\u001a\u000200H\u0016J\r\u00101\u001a\u00020\tH\u0010\u00a2\u0006\u0002\u00082J\u0008\u00103\u001a\u00020\tH\u0016J\u0008\u00104\u001a\u00020\rH\u0016J3\u00105\u001a\u0002002\u0006\u00106\u001a\u00020\u00032\u0014\u00107\u001a\u0010\u0012\u0004\u0012\u000209\u0012\u0004\u0012\u000209\u0018\u0001082\u0006\u0010:\u001a\u00020\u0005H\u0000\u00a2\u0006\u0002\u0008;J\u0015\u0010<\u001a\u00020\t2\u0006\u0010=\u001a\u00020\u0001H\u0010\u00a2\u0006\u0002\u0008>J\u0015\u0010?\u001a\u00020\t2\u0006\u0010=\u001a\u00020\u0001H\u0010\u00a2\u0006\u0002\u0008@J\r\u0010A\u001a\u00020\tH\u0010\u00a2\u0006\u0002\u0008BJ\u0015\u0010C\u001a\u00020\t2\u0006\u0010D\u001a\u00020\u0014H\u0010\u00a2\u0006\u0002\u0008EJ\u0015\u0010F\u001a\u00020\t2\u0006\u0010\u0002\u001a\u00020\u0003H\u0000\u00a2\u0006\u0002\u0008GJ\u0015\u0010H\u001a\u00020\t2\u0006\u0010&\u001a\u00020\u0005H\u0000\u00a2\u0006\u0002\u0008IJ8\u0010J\u001a\u00020\u00002\u0016\u0008\u0002\u0010\u0006\u001a\u0010\u0012\u0004\u0012\u00020\u0008\u0012\u0004\u0012\u00020\t\u0018\u00010\u00072\u0016\u0008\u0002\u0010\n\u001a\u0010\u0012\u0004\u0012\u00020\u0008\u0012\u0004\u0012\u00020\t\u0018\u00010\u0007H\u0016J\u001e\u0010K\u001a\u00020\u00012\u0014\u0010\u0006\u001a\u0010\u0012\u0004\u0012\u00020\u0008\u0012\u0004\u0012\u00020\t\u0018\u00010\u0007H\u0016J\r\u0010L\u001a\u00020\tH\u0000\u00a2\u0006\u0002\u0008MR\u001a\u0010\u000c\u001a\u00020\rX\u0080\u000e\u00a2\u0006\u000e\n\u0000\u001a\u0004\u0008\u000e\u0010\u000f\"\u0004\u0008\u0010\u0010\u0011R4\u0010\u0015\u001a\n\u0012\u0004\u0012\u00020\u0014\u0018\u00010\u00132\u000e\u0010\u0012\u001a\n\u0012\u0004\u0012\u00020\u0014\u0018\u00010\u0013@VX\u0090\u000e\u00a2\u0006\u000e\n\u0000\u001a\u0004\u0008\u0016\u0010\u0017\"\u0004\u0008\u0018\u0010\u0019R\u001a\u0010\u001a\u001a\u00020\u0005X\u0080\u000e\u00a2\u0006\u000e\n\u0000\u001a\u0004\u0008\u001b\u0010\u001c\"\u0004\u0008\u001d\u0010\u001eR\"\u0010\u0006\u001a\u0010\u0012\u0004\u0012\u00020\u0008\u0012\u0004\u0012\u00020\t\u0018\u00010\u0007X\u0090\u0004\u00a2\u0006\u0008\n\u0000\u001a\u0004\u0008\u001f\u0010 R\u0014\u0010!\u001a\u00020\r8VX\u0096\u0004\u00a2\u0006\u0006\u001a\u0004\u0008\"\u0010\u000fR\u0014\u0010#\u001a\u00020\u00018VX\u0096\u0004\u00a2\u0006\u0006\u001a\u0004\u0008$\u0010%R\u000e\u0010&\u001a\u00020\u0003X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\"\u0010\n\u001a\u0010\u0012\u0004\u0012\u00020\u0008\u0012\u0004\u0012\u00020\t\u0018\u00010\u0007X\u0090\u0004\u00a2\u0006\u0008\n\u0000\u001a\u0004\u0008\'\u0010 \u0082\u0002\u0007\n\u0005\u0008\u009920\u0001\u00a8\u0006N"
    }
    d2 = {
        "Landroidx/compose/runtime/snapshots/MutableSnapshot;",
        "Landroidx/compose/runtime/snapshots/Snapshot;",
        "id",
        "",
        "invalid",
        "Landroidx/compose/runtime/snapshots/SnapshotIdSet;",
        "readObserver",
        "Lkotlin/Function1;",
        "",
        "",
        "writeObserver",
        "(ILandroidx/compose/runtime/snapshots/SnapshotIdSet;Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;)V",
        "applied",
        "",
        "getApplied$runtime_release",
        "()Z",
        "setApplied$runtime_release",
        "(Z)V",
        "<set-?>",
        "",
        "Landroidx/compose/runtime/snapshots/StateObject;",
        "modified",
        "getModified$runtime_release",
        "()Ljava/util/Set;",
        "setModified",
        "(Ljava/util/Set;)V",
        "previousIds",
        "getPreviousIds$runtime_release",
        "()Landroidx/compose/runtime/snapshots/SnapshotIdSet;",
        "setPreviousIds$runtime_release",
        "(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V",
        "getReadObserver$runtime_release",
        "()Lkotlin/jvm/functions/Function1;",
        "readOnly",
        "getReadOnly",
        "root",
        "getRoot",
        "()Landroidx/compose/runtime/snapshots/Snapshot;",
        "snapshots",
        "getWriteObserver$runtime_release",
        "abandon",
        "advance",
        "advance$runtime_release",
        "T",
        "block",
        "Lkotlin/Function0;",
        "(Lkotlin/jvm/functions/Function0;)Ljava/lang/Object;",
        "apply",
        "Landroidx/compose/runtime/snapshots/SnapshotApplyResult;",
        "close",
        "close$runtime_release",
        "dispose",
        "hasPendingChanges",
        "innerApply",
        "snapshotId",
        "optimisticMerges",
        "",
        "Landroidx/compose/runtime/snapshots/StateRecord;",
        "invalidSnapshots",
        "innerApply$runtime_release",
        "nestedActivated",
        "snapshot",
        "nestedActivated$runtime_release",
        "nestedDeactivated",
        "nestedDeactivated$runtime_release",
        "notifyObjectsInitialized",
        "notifyObjectsInitialized$runtime_release",
        "recordModified",
        "state",
        "recordModified$runtime_release",
        "recordPrevious",
        "recordPrevious$runtime_release",
        "recordPreviousList",
        "recordPreviousList$runtime_release",
        "takeNestedMutableSnapshot",
        "takeNestedSnapshot",
        "validateNotApplied",
        "validateNotApplied$runtime_release",
        "runtime_release"
    }
    k = 0x1
    mv = {
        0x1,
        0x5,
        0x1
    }
    xi = 0x30
.end annotation


# static fields
.field public static final $stable:I


# instance fields
.field private applied:Z

.field private modified:Ljava/util/Set;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/Set<",
            "Landroidx/compose/runtime/snapshots/StateObject;",
            ">;"
        }
    .end annotation
.end field

.field private previousIds:Landroidx/compose/runtime/snapshots/SnapshotIdSet;

.field private final readObserver:Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field

.field private snapshots:I

.field private final writeObserver:Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method static constructor <clinit>()V
    .locals 1

    const/16 v0, 0x8

    sput v0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->$stable:I

    return-void
.end method

.method public constructor <init>(ILandroidx/compose/runtime/snapshots/SnapshotIdSet;Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;)V
    .locals 1
    .param p1, "id"    # I
    .param p2, "invalid"    # Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    .param p3, "readObserver"    # Lkotlin/jvm/functions/Function1;
    .param p4, "writeObserver"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(I",
            "Landroidx/compose/runtime/snapshots/SnapshotIdSet;",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;)V"
        }
    .end annotation

    const-string v0, "invalid"

    invoke-static {p2, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 509
    const/4 v0, 0x0

    invoke-direct {p0, p1, p2, v0}, Landroidx/compose/runtime/snapshots/Snapshot;-><init>(ILandroidx/compose/runtime/snapshots/SnapshotIdSet;Lkotlin/jvm/internal/DefaultConstructorMarker;)V

    .line 507
    iput-object p3, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->readObserver:Lkotlin/jvm/functions/Function1;

    .line 508
    iput-object p4, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->writeObserver:Lkotlin/jvm/functions/Function1;

    .line 853
    sget-object v0, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->Companion:Landroidx/compose/runtime/snapshots/SnapshotIdSet$Companion;

    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/SnapshotIdSet$Companion;->getEMPTY()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v0

    iput-object v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->previousIds:Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    .line 859
    const/4 v0, 0x1

    iput v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->snapshots:I

    .line 504
    return-void
.end method

.method private final abandon()V
    .locals 7

    .line 702
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getModified$runtime_release()Ljava/util/Set;

    move-result-object v0

    .line 703
    .local v0, "modified":Ljava/util/Set;
    if-eqz v0, :cond_3

    .line 704
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->validateNotApplied$runtime_release()V

    .line 708
    const/4 v1, 0x0

    invoke-virtual {p0, v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setModified(Ljava/util/Set;)V

    .line 709
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v1

    .line 710
    .local v1, "id":I
    invoke-interface {v0}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object v2

    :cond_0
    invoke-interface {v2}, Ljava/util/Iterator;->hasNext()Z

    move-result v3

    if-eqz v3, :cond_3

    invoke-interface {v2}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Landroidx/compose/runtime/snapshots/StateObject;

    .line 711
    .local v3, "state":Landroidx/compose/runtime/snapshots/StateObject;
    invoke-interface {v3}, Landroidx/compose/runtime/snapshots/StateObject;->getFirstStateRecord()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v4

    .line 712
    .local v4, "current":Landroidx/compose/runtime/snapshots/StateRecord;
    :goto_0
    if-eqz v4, :cond_0

    .line 713
    invoke-virtual {v4}, Landroidx/compose/runtime/snapshots/StateRecord;->getSnapshotId$runtime_release()I

    move-result v5

    if-eq v5, v1, :cond_1

    iget-object v5, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->previousIds:Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    check-cast v5, Ljava/lang/Iterable;

    invoke-virtual {v4}, Landroidx/compose/runtime/snapshots/StateRecord;->getSnapshotId$runtime_release()I

    move-result v6

    invoke-static {v6}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v6

    invoke-static {v5, v6}, Lkotlin/collections/CollectionsKt;->contains(Ljava/lang/Iterable;Ljava/lang/Object;)Z

    move-result v5

    if-eqz v5, :cond_2

    .line 714
    :cond_1
    const/4 v5, 0x0

    invoke-virtual {v4, v5}, Landroidx/compose/runtime/snapshots/StateRecord;->setSnapshotId$runtime_release(I)V

    .line 716
    :cond_2
    invoke-virtual {v4}, Landroidx/compose/runtime/snapshots/StateRecord;->getNext$runtime_release()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v4

    goto :goto_0

    .line 722
    .end local v1    # "id":I
    .end local v3    # "state":Landroidx/compose/runtime/snapshots/StateObject;
    .end local v4    # "current":Landroidx/compose/runtime/snapshots/StateRecord;
    :cond_3
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->close$runtime_release()V

    .line 723
    return-void
.end method

.method public static synthetic takeNestedMutableSnapshot$default(Landroidx/compose/runtime/snapshots/MutableSnapshot;Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;ILjava/lang/Object;)Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .locals 1

    .line 529
    if-nez p4, :cond_2

    and-int/lit8 p4, p3, 0x1

    const/4 v0, 0x0

    if-eqz p4, :cond_0

    .line 530
    move-object p1, v0

    .line 529
    :cond_0
    and-int/lit8 p3, p3, 0x2

    if-eqz p3, :cond_1

    .line 531
    move-object p2, v0

    .line 529
    :cond_1
    invoke-virtual {p0, p1, p2}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->takeNestedMutableSnapshot(Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;)Landroidx/compose/runtime/snapshots/MutableSnapshot;

    move-result-object p0

    return-object p0

    :cond_2
    new-instance p0, Ljava/lang/UnsupportedOperationException;

    const-string p1, "Super calls with default arguments not supported in this target, function: takeNestedMutableSnapshot"

    invoke-direct {p0, p1}, Ljava/lang/UnsupportedOperationException;-><init>(Ljava/lang/String;)V

    throw p0
.end method


# virtual methods
.method public final advance$runtime_release(Lkotlin/jvm/functions/Function0;)Ljava/lang/Object;
    .locals 12
    .param p1, "block"    # Lkotlin/jvm/functions/Function0;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "<T:",
            "Ljava/lang/Object;",
            ">(",
            "Lkotlin/jvm/functions/Function0<",
            "+TT;>;)TT;"
        }
    .end annotation

    const-string v0, "block"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const/4 v0, 0x0

    .line 814
    .local v0, "$i$f$advance$runtime_release":I
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v1

    invoke-virtual {p0, v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->recordPrevious$runtime_release(I)V

    .line 815
    invoke-interface {p1}, Lkotlin/jvm/functions/Function0;->invoke()Ljava/lang/Object;

    move-result-object v1

    move-object v2, v1

    .local v2, "it":Ljava/lang/Object;
    const/4 v3, 0x0

    .line 816
    .local v3, "$i$a$-also-MutableSnapshot$advance$1":I
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v4

    .line 817
    .local v4, "previousId":I
    const/4 v5, 0x0

    .line 1825
    .local v5, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v6

    .local v6, "lock$iv$iv":Ljava/lang/Object;
    const/4 v7, 0x0

    .line 1826
    .local v7, "$i$f$synchronized":I
    monitor-enter v6

    const/4 v8, 0x0

    .line 818
    .local v8, "$i$a$-sync-MutableSnapshot$advance$1$1":I
    const/4 v9, 0x1

    :try_start_0
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getNextSnapshotId$p()I

    move-result v10

    add-int/lit8 v11, v10, 0x1

    invoke-static {v11}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setNextSnapshotId$p(I)V

    invoke-virtual {p0, v10}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setId$runtime_release(I)V

    .line 819
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v10

    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v11

    invoke-virtual {v10, v11}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v10

    invoke-static {v10}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setOpenSnapshots$p(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 820
    nop

    .end local v8    # "$i$a$-sync-MutableSnapshot$advance$1$1":I
    sget-object v8, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    invoke-static {v9}, Lkotlin/jvm/internal/InlineMarker;->finallyStart(I)V

    monitor-exit v6

    invoke-static {v9}, Lkotlin/jvm/internal/InlineMarker;->finallyEnd(I)V

    .line 1826
    nop

    .line 1825
    .end local v6    # "lock$iv$iv":Ljava/lang/Object;
    .end local v7    # "$i$f$synchronized":I
    nop

    .line 821
    .end local v5    # "$i$f$sync":I
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v5

    .line 822
    .local v5, "currentInvalid":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    add-int/lit8 v6, v4, 0x1

    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v7

    if-ge v6, v7, :cond_1

    :cond_0
    move v8, v6

    .local v8, "invalidId":I
    add-int/2addr v6, v9

    .line 823
    invoke-virtual {v5, v8}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v5

    .line 822
    if-lt v6, v7, :cond_0

    .line 825
    .end local v8    # "invalidId":I
    :cond_1
    invoke-virtual {p0, v5}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setInvalid$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 826
    nop

    .line 815
    .end local v2    # "it":Ljava/lang/Object;
    .end local v3    # "$i$a$-also-MutableSnapshot$advance$1":I
    .end local v4    # "previousId":I
    .end local v5    # "currentInvalid":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    return-object v1

    .line 820
    .restart local v2    # "it":Ljava/lang/Object;
    .restart local v3    # "$i$a$-also-MutableSnapshot$advance$1":I
    .restart local v4    # "previousId":I
    .local v5, "$i$f$sync":I
    .restart local v6    # "lock$iv$iv":Ljava/lang/Object;
    .restart local v7    # "$i$f$synchronized":I
    :catchall_0
    move-exception v1

    invoke-static {v9}, Lkotlin/jvm/internal/InlineMarker;->finallyStart(I)V

    monitor-exit v6

    invoke-static {v9}, Lkotlin/jvm/internal/InlineMarker;->finallyEnd(I)V

    throw v1
.end method

.method public final advance$runtime_release()V
    .locals 11

    .line 829
    move-object v0, p0

    .local v0, "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    const/4 v1, 0x0

    .line 1827
    .local v1, "$i$f$advance$runtime_release":I
    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v2

    invoke-virtual {v0, v2}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->recordPrevious$runtime_release(I)V

    .line 1828
    const/4 v2, 0x0

    .line 829
    .local v2, "$i$a$-advance$runtime_release-MutableSnapshot$advance$2":I
    nop

    .end local v2    # "$i$a$-advance$runtime_release-MutableSnapshot$advance$2":I
    sget-object v2, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .local v2, "it$iv":Ljava/lang/Object;
    const/4 v3, 0x0

    .line 1829
    .local v3, "$i$a$-also-MutableSnapshot$advance$1$iv":I
    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v4

    .line 1830
    .local v4, "previousId$iv":I
    const/4 v5, 0x0

    .line 1831
    .local v5, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v6

    .local v6, "lock$iv$iv$iv":Ljava/lang/Object;
    const/4 v7, 0x0

    .line 1832
    .local v7, "$i$f$synchronized":I
    monitor-enter v6

    const/4 v8, 0x0

    .line 1833
    .local v8, "$i$a$-sync-MutableSnapshot$advance$1$1$iv":I
    :try_start_0
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getNextSnapshotId$p()I

    move-result v9

    add-int/lit8 v10, v9, 0x1

    invoke-static {v10}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setNextSnapshotId$p(I)V

    invoke-virtual {v0, v9}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setId$runtime_release(I)V

    .line 1834
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9

    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v10

    invoke-virtual {v9, v10}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9

    invoke-static {v9}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setOpenSnapshots$p(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 1835
    nop

    .end local v8    # "$i$a$-sync-MutableSnapshot$advance$1$1$iv":I
    sget-object v8, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit v6

    .line 1832
    nop

    .line 1831
    .end local v6    # "lock$iv$iv$iv":Ljava/lang/Object;
    .end local v7    # "$i$f$synchronized":I
    nop

    .line 1836
    .end local v5    # "$i$f$sync":I
    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v5

    .line 1837
    .local v5, "currentInvalid$iv":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    add-int/lit8 v6, v4, 0x1

    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v7

    if-ge v6, v7, :cond_1

    :cond_0
    move v8, v6

    .local v8, "invalidId$iv":I
    add-int/lit8 v6, v6, 0x1

    .line 1838
    invoke-virtual {v5, v8}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v5

    .line 1837
    if-lt v6, v7, :cond_0

    .line 1840
    .end local v8    # "invalidId$iv":I
    :cond_1
    invoke-virtual {v0, v5}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setInvalid$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 1841
    nop

    .line 1828
    .end local v2    # "it$iv":Ljava/lang/Object;
    .end local v3    # "$i$a$-also-MutableSnapshot$advance$1$iv":I
    .end local v4    # "previousId$iv":I
    .end local v5    # "currentInvalid$iv":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    nop

    .line 829
    .end local v0    # "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .end local v1    # "$i$f$advance$runtime_release":I
    return-void

    .line 1835
    .restart local v0    # "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .restart local v1    # "$i$f$advance$runtime_release":I
    .restart local v2    # "it$iv":Ljava/lang/Object;
    .restart local v3    # "$i$a$-also-MutableSnapshot$advance$1$iv":I
    .restart local v4    # "previousId$iv":I
    .local v5, "$i$f$sync":I
    .restart local v6    # "lock$iv$iv$iv":Ljava/lang/Object;
    .restart local v7    # "$i$f$synchronized":I
    :catchall_0
    move-exception v8

    monitor-exit v6

    throw v8
.end method

.method public apply()Landroidx/compose/runtime/snapshots/SnapshotApplyResult;
    .locals 14

    .line 578
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getModified$runtime_release()Ljava/util/Set;

    move-result-object v0

    .line 579
    .local v0, "modified":Ljava/util/Set;
    const/4 v1, 0x0

    if-eqz v0, :cond_0

    .line 580
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getCurrentGlobalSnapshot$p()Ljava/util/concurrent/atomic/AtomicReference;

    move-result-object v2

    invoke-virtual {v2}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v2

    const-string v3, "currentGlobalSnapshot.get()"

    invoke-static {v2, v3}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    check-cast v2, Landroidx/compose/runtime/snapshots/MutableSnapshot;

    .line 581
    nop

    .line 582
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v3

    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getCurrentGlobalSnapshot$p()Ljava/util/concurrent/atomic/AtomicReference;

    move-result-object v4

    invoke-virtual {v4}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v4

    check-cast v4, Landroidx/compose/runtime/snapshots/GlobalSnapshot;

    invoke-virtual {v4}, Landroidx/compose/runtime/snapshots/GlobalSnapshot;->getId()I

    move-result v4

    invoke-virtual {v3, v4}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->clear(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v3

    .line 579
    invoke-static {v2, p0, v3}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$optimisticMerges(Landroidx/compose/runtime/snapshots/MutableSnapshot;Landroidx/compose/runtime/snapshots/MutableSnapshot;Landroidx/compose/runtime/snapshots/SnapshotIdSet;)Ljava/util/Map;

    move-result-object v2

    goto :goto_0

    .line 583
    :cond_0
    move-object v2, v1

    .line 579
    :goto_0
    nop

    .line 584
    .local v2, "optimisticMerges":Ljava/util/Map;
    const/4 v3, 0x0

    .line 1785
    .local v3, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v4

    .local v4, "lock$iv$iv":Ljava/lang/Object;
    const/4 v5, 0x0

    .line 1786
    .local v5, "$i$f$synchronized":I
    monitor-enter v4

    const/4 v6, 0x0

    .line 585
    .local v6, "$i$a$-sync-MutableSnapshot$apply$1":I
    :try_start_0
    move-object v7, p0

    check-cast v7, Landroidx/compose/runtime/snapshots/Snapshot;

    invoke-static {v7}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$validateOpen(Landroidx/compose/runtime/snapshots/Snapshot;)V

    .line 586
    const/4 v7, 0x1

    if-eqz v0, :cond_3

    invoke-interface {v0}, Ljava/util/Set;->size()I

    move-result v8

    if-nez v8, :cond_1

    goto :goto_1

    .line 596
    :cond_1
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getCurrentGlobalSnapshot$p()Ljava/util/concurrent/atomic/AtomicReference;

    move-result-object v8

    invoke-virtual {v8}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v8

    check-cast v8, Landroidx/compose/runtime/snapshots/GlobalSnapshot;

    .line 597
    .local v8, "previousGlobalSnapshot":Landroidx/compose/runtime/snapshots/GlobalSnapshot;
    nop

    .line 598
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getNextSnapshotId$p()I

    move-result v9

    .line 599
    nop

    .line 600
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v10

    invoke-virtual {v8}, Landroidx/compose/runtime/snapshots/GlobalSnapshot;->getId()I

    move-result v11

    invoke-virtual {v10, v11}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->clear(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v10

    .line 597
    invoke-virtual {p0, v9, v2, v10}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->innerApply$runtime_release(ILjava/util/Map;Landroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/SnapshotApplyResult;

    move-result-object v9

    .line 602
    .local v9, "result":Landroidx/compose/runtime/snapshots/SnapshotApplyResult;
    sget-object v10, Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Success;->INSTANCE:Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Success;

    invoke-static {v9, v10}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v10
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    if-nez v10, :cond_2

    .end local v3    # "$i$f$sync":I
    .end local v4    # "lock$iv$iv":Ljava/lang/Object;
    .end local v5    # "$i$f$synchronized":I
    .end local v6    # "$i$a$-sync-MutableSnapshot$apply$1":I
    .end local v8    # "previousGlobalSnapshot":Landroidx/compose/runtime/snapshots/GlobalSnapshot;
    .end local v9    # "result":Landroidx/compose/runtime/snapshots/SnapshotApplyResult;
    monitor-exit v4

    return-object v9

    .line 605
    .restart local v3    # "$i$f$sync":I
    .restart local v4    # "lock$iv$iv":Ljava/lang/Object;
    .restart local v5    # "$i$f$synchronized":I
    .restart local v6    # "$i$a$-sync-MutableSnapshot$apply$1":I
    .restart local v8    # "previousGlobalSnapshot":Landroidx/compose/runtime/snapshots/GlobalSnapshot;
    .restart local v9    # "result":Landroidx/compose/runtime/snapshots/SnapshotApplyResult;
    :cond_2
    :try_start_1
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->close$runtime_release()V

    .line 608
    const-string v10, "previousGlobalSnapshot"

    invoke-static {v8, v10}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    move-object v10, v8

    check-cast v10, Landroidx/compose/runtime/snapshots/Snapshot;

    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getEmptyLambda$p()Lkotlin/jvm/functions/Function1;

    move-result-object v11

    invoke-static {v10, v11}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$takeNewGlobalSnapshot(Landroidx/compose/runtime/snapshots/Snapshot;Lkotlin/jvm/functions/Function1;)Ljava/lang/Object;

    .line 609
    invoke-virtual {v8}, Landroidx/compose/runtime/snapshots/GlobalSnapshot;->getModified$runtime_release()Ljava/util/Set;

    move-result-object v10

    .line 610
    .local v10, "globalModified":Ljava/util/Set;
    invoke-virtual {p0, v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setModified(Ljava/util/Set;)V

    .line 611
    invoke-virtual {v8, v1}, Landroidx/compose/runtime/snapshots/GlobalSnapshot;->setModified(Ljava/util/Set;)V

    .line 613
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getApplyObservers$p()Ljava/util/List;

    move-result-object v1

    check-cast v1, Ljava/util/Collection;

    invoke-static {v1}, Lkotlin/collections/CollectionsKt;->toMutableList(Ljava/util/Collection;)Ljava/util/List;

    move-result-object v1

    invoke-static {v1, v10}, Lkotlin/TuplesKt;->to(Ljava/lang/Object;Ljava/lang/Object;)Lkotlin/Pair;

    move-result-object v1

    goto :goto_2

    .line 587
    .end local v8    # "previousGlobalSnapshot":Landroidx/compose/runtime/snapshots/GlobalSnapshot;
    .end local v9    # "result":Landroidx/compose/runtime/snapshots/SnapshotApplyResult;
    .end local v10    # "globalModified":Ljava/util/Set;
    :cond_3
    :goto_1
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->close$runtime_release()V

    .line 588
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getCurrentGlobalSnapshot$p()Ljava/util/concurrent/atomic/AtomicReference;

    move-result-object v8

    invoke-virtual {v8}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v8

    check-cast v8, Landroidx/compose/runtime/snapshots/GlobalSnapshot;

    .line 589
    .restart local v8    # "previousGlobalSnapshot":Landroidx/compose/runtime/snapshots/GlobalSnapshot;
    const-string v9, "previousGlobalSnapshot"

    invoke-static {v8, v9}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    move-object v9, v8

    check-cast v9, Landroidx/compose/runtime/snapshots/Snapshot;

    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getEmptyLambda$p()Lkotlin/jvm/functions/Function1;

    move-result-object v10

    invoke-static {v9, v10}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$takeNewGlobalSnapshot(Landroidx/compose/runtime/snapshots/Snapshot;Lkotlin/jvm/functions/Function1;)Ljava/lang/Object;

    .line 590
    invoke-virtual {v8}, Landroidx/compose/runtime/snapshots/GlobalSnapshot;->getModified$runtime_release()Ljava/util/Set;

    move-result-object v9

    .line 591
    .local v9, "globalModified":Ljava/util/Set;
    if-eqz v9, :cond_4

    move-object v10, v9

    check-cast v10, Ljava/util/Collection;

    invoke-interface {v10}, Ljava/util/Collection;->isEmpty()Z

    move-result v10

    xor-int/2addr v10, v7

    if-eqz v10, :cond_4

    .line 592
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getApplyObservers$p()Ljava/util/List;

    move-result-object v1

    check-cast v1, Ljava/util/Collection;

    invoke-static {v1}, Lkotlin/collections/CollectionsKt;->toMutableList(Ljava/util/Collection;)Ljava/util/List;

    move-result-object v1

    invoke-static {v1, v9}, Lkotlin/TuplesKt;->to(Ljava/lang/Object;Ljava/lang/Object;)Lkotlin/Pair;

    move-result-object v1

    goto :goto_2

    .line 594
    :cond_4
    invoke-static {}, Lkotlin/collections/CollectionsKt;->emptyList()Ljava/util/List;

    move-result-object v10

    invoke-static {v10, v1}, Lkotlin/TuplesKt;->to(Ljava/lang/Object;Ljava/lang/Object;)Lkotlin/Pair;

    move-result-object v1
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    .line 586
    .end local v8    # "previousGlobalSnapshot":Landroidx/compose/runtime/snapshots/GlobalSnapshot;
    .end local v9    # "globalModified":Ljava/util/Set;
    :goto_2
    nop

    .end local v6    # "$i$a$-sync-MutableSnapshot$apply$1":I
    monitor-exit v4

    .line 1786
    nop

    .line 1785
    .end local v4    # "lock$iv$iv":Ljava/lang/Object;
    .end local v5    # "$i$f$synchronized":I
    nop

    .line 584
    .end local v3    # "$i$f$sync":I
    invoke-virtual {v1}, Lkotlin/Pair;->component1()Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Ljava/util/List;

    .local v3, "observers":Ljava/util/List;
    invoke-virtual {v1}, Lkotlin/Pair;->component2()Ljava/lang/Object;

    move-result-object v1

    check-cast v1, Ljava/util/Set;

    .line 618
    .local v1, "globalModified":Ljava/util/Set;
    iput-boolean v7, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->applied:Z

    .line 621
    const/4 v4, 0x0

    if-eqz v1, :cond_7

    move-object v5, v1

    check-cast v5, Ljava/util/Collection;

    invoke-interface {v5}, Ljava/util/Collection;->isEmpty()Z

    move-result v5

    xor-int/2addr v5, v7

    if-eqz v5, :cond_7

    .line 622
    move-object v5, v3

    .local v5, "$this$fastForEach$iv":Ljava/util/List;
    const/4 v6, 0x0

    .line 1787
    .local v6, "$i$f$fastForEach":I
    nop

    .line 1788
    invoke-interface {v5}, Ljava/util/List;->size()I

    move-result v8

    add-int/lit8 v8, v8, -0x1

    if-ltz v8, :cond_6

    move v9, v4

    :cond_5
    move v10, v9

    .local v10, "index$iv":I
    add-int/2addr v9, v7

    .line 1789
    invoke-interface {v5, v10}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v11

    .line 1790
    .local v11, "item$iv":Ljava/lang/Object;
    move-object v12, v11

    check-cast v12, Lkotlin/jvm/functions/Function2;

    .local v12, "it":Lkotlin/jvm/functions/Function2;
    const/4 v13, 0x0

    .line 623
    .local v13, "$i$a$-fastForEach-MutableSnapshot$apply$2":I
    invoke-interface {v12, v1, p0}, Lkotlin/jvm/functions/Function2;->invoke(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    .line 624
    nop

    .line 1788
    .end local v11    # "item$iv":Ljava/lang/Object;
    .end local v12    # "it":Lkotlin/jvm/functions/Function2;
    .end local v13    # "$i$a$-fastForEach-MutableSnapshot$apply$2":I
    if-le v9, v8, :cond_5

    .line 1792
    .end local v10    # "index$iv":I
    :cond_6
    nop

    .line 627
    .end local v5    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v6    # "$i$f$fastForEach":I
    :cond_7
    if-eqz v0, :cond_a

    move-object v5, v0

    check-cast v5, Ljava/util/Collection;

    invoke-interface {v5}, Ljava/util/Collection;->isEmpty()Z

    move-result v5

    xor-int/2addr v5, v7

    if-eqz v5, :cond_a

    .line 628
    move-object v5, v3

    .restart local v5    # "$this$fastForEach$iv":Ljava/util/List;
    const/4 v6, 0x0

    .line 1793
    .restart local v6    # "$i$f$fastForEach":I
    nop

    .line 1794
    invoke-interface {v5}, Ljava/util/List;->size()I

    move-result v8

    add-int/lit8 v8, v8, -0x1

    if-ltz v8, :cond_9

    :cond_8
    move v9, v4

    .local v9, "index$iv":I
    add-int/2addr v4, v7

    .line 1795
    invoke-interface {v5, v9}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v10

    .line 1796
    .local v10, "item$iv":Ljava/lang/Object;
    move-object v11, v10

    check-cast v11, Lkotlin/jvm/functions/Function2;

    .local v11, "it":Lkotlin/jvm/functions/Function2;
    const/4 v12, 0x0

    .line 629
    .local v12, "$i$a$-fastForEach-MutableSnapshot$apply$3":I
    invoke-interface {v11, v0, p0}, Lkotlin/jvm/functions/Function2;->invoke(Ljava/lang/Object;Ljava/lang/Object;)Ljava/lang/Object;

    .line 630
    nop

    .line 1794
    .end local v10    # "item$iv":Ljava/lang/Object;
    .end local v11    # "it":Lkotlin/jvm/functions/Function2;
    .end local v12    # "$i$a$-fastForEach-MutableSnapshot$apply$3":I
    if-le v4, v8, :cond_8

    .line 1798
    .end local v9    # "index$iv":I
    :cond_9
    nop

    .line 633
    .end local v5    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v6    # "$i$f$fastForEach":I
    :cond_a
    sget-object v4, Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Success;->INSTANCE:Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Success;

    check-cast v4, Landroidx/compose/runtime/snapshots/SnapshotApplyResult;

    return-object v4

    .line 586
    .end local v1    # "globalModified":Ljava/util/Set;
    .local v3, "$i$f$sync":I
    .restart local v4    # "lock$iv$iv":Ljava/lang/Object;
    .local v5, "$i$f$synchronized":I
    :catchall_0
    move-exception v1

    monitor-exit v4

    throw v1
.end method

.method public close$runtime_release()V
    .locals 6

    .line 686
    const/4 v0, 0x0

    .line 1816
    .local v0, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v1

    .local v1, "lock$iv$iv":Ljava/lang/Object;
    const/4 v2, 0x0

    .line 1817
    .local v2, "$i$f$synchronized":I
    monitor-enter v1

    const/4 v3, 0x0

    .line 688
    .local v3, "$i$a$-sync-MutableSnapshot$close$1":I
    :try_start_0
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v5

    invoke-virtual {v4, v5}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->clear(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getPreviousIds$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v5

    invoke-virtual {v4, v5}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->andNot(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-static {v4}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setOpenSnapshots$p(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 689
    nop

    .end local v3    # "$i$a$-sync-MutableSnapshot$close$1":I
    sget-object v3, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit v1

    .line 1817
    nop

    .line 1816
    .end local v1    # "lock$iv$iv":Ljava/lang/Object;
    .end local v2    # "$i$f$synchronized":I
    nop

    .line 690
    .end local v0    # "$i$f$sync":I
    return-void

    .line 689
    .restart local v0    # "$i$f$sync":I
    .restart local v1    # "lock$iv$iv":Ljava/lang/Object;
    .restart local v2    # "$i$f$synchronized":I
    :catchall_0
    move-exception v3

    monitor-exit v1

    throw v3
.end method

.method public dispose()V
    .locals 1

    .line 641
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getDisposed$runtime_release()Z

    move-result v0

    if-nez v0, :cond_0

    .line 642
    invoke-super {p0}, Landroidx/compose/runtime/snapshots/Snapshot;->dispose()V

    .line 643
    move-object v0, p0

    check-cast v0, Landroidx/compose/runtime/snapshots/Snapshot;

    invoke-virtual {p0, v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->nestedDeactivated$runtime_release(Landroidx/compose/runtime/snapshots/Snapshot;)V

    .line 645
    :cond_0
    return-void
.end method

.method public final getApplied$runtime_release()Z
    .locals 1

    .line 864
    iget-boolean v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->applied:Z

    return v0
.end method

.method public getModified$runtime_release()Ljava/util/Set;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()",
            "Ljava/util/Set<",
            "Landroidx/compose/runtime/snapshots/StateObject;",
            ">;"
        }
    .end annotation

    .line 847
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->modified:Ljava/util/Set;

    return-object v0
.end method

.method public final getPreviousIds$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    .locals 1

    .line 853
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->previousIds:Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    return-object v0
.end method

.method public getReadObserver$runtime_release()Lkotlin/jvm/functions/Function1;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation

    .line 507
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->readObserver:Lkotlin/jvm/functions/Function1;

    return-object v0
.end method

.method public getReadOnly()Z
    .locals 1

    .line 636
    const/4 v0, 0x0

    return v0
.end method

.method public getRoot()Landroidx/compose/runtime/snapshots/Snapshot;
    .locals 1

    .line 638
    move-object v0, p0

    check-cast v0, Landroidx/compose/runtime/snapshots/Snapshot;

    return-object v0
.end method

.method public getWriteObserver$runtime_release()Lkotlin/jvm/functions/Function1;
    .locals 1
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;"
        }
    .end annotation

    .line 508
    iget-object v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->writeObserver:Lkotlin/jvm/functions/Function1;

    return-object v0
.end method

.method public hasPendingChanges()Z
    .locals 3

    .line 514
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getModified$runtime_release()Ljava/util/Set;

    move-result-object v0

    const/4 v1, 0x0

    if-nez v0, :cond_1

    :cond_0
    goto :goto_0

    :cond_1
    check-cast v0, Ljava/util/Collection;

    invoke-interface {v0}, Ljava/util/Collection;->isEmpty()Z

    move-result v0

    const/4 v2, 0x1

    xor-int/2addr v0, v2

    if-ne v0, v2, :cond_0

    move v1, v2

    :goto_0
    return v1
.end method

.method public final innerApply$runtime_release(ILjava/util/Map;Landroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/SnapshotApplyResult;
    .locals 23
    .param p1, "snapshotId"    # I
    .param p2, "optimisticMerges"    # Ljava/util/Map;
    .param p3, "invalidSnapshots"    # Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(I",
            "Ljava/util/Map<",
            "Landroidx/compose/runtime/snapshots/StateRecord;",
            "+",
            "Landroidx/compose/runtime/snapshots/StateRecord;",
            ">;",
            "Landroidx/compose/runtime/snapshots/SnapshotIdSet;",
            ")",
            "Landroidx/compose/runtime/snapshots/SnapshotApplyResult;"
        }
    .end annotation

    move-object/from16 v1, p0

    move-object/from16 v2, p2

    move-object/from16 v3, p3

    const-string v0, "invalidSnapshots"

    invoke-static {v3, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 741
    const/4 v0, 0x0

    .line 742
    .local v0, "mergedRecords":Ljava/lang/Object;
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v5

    invoke-virtual {v4, v5}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    iget-object v5, v1, Landroidx/compose/runtime/snapshots/MutableSnapshot;->previousIds:Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    invoke-virtual {v4, v5}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->or(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    .line 743
    .local v4, "start":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getModified$runtime_release()Ljava/util/Set;

    move-result-object v5

    invoke-static {v5}, Lkotlin/jvm/internal/Intrinsics;->checkNotNull(Ljava/lang/Object;)V

    .line 744
    .local v5, "modified":Ljava/util/Set;
    const/4 v6, 0x0

    .line 745
    .local v6, "statesToRemove":Ljava/lang/Object;
    invoke-interface {v5}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object v7

    move-object v8, v6

    move-object v6, v0

    .end local v0    # "mergedRecords":Ljava/lang/Object;
    .local v6, "mergedRecords":Ljava/lang/Object;
    .local v8, "statesToRemove":Ljava/lang/Object;
    :goto_0
    invoke-interface {v7}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_d

    invoke-interface {v7}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Landroidx/compose/runtime/snapshots/StateObject;

    .line 746
    .local v0, "state":Landroidx/compose/runtime/snapshots/StateObject;
    invoke-interface {v0}, Landroidx/compose/runtime/snapshots/StateObject;->getFirstStateRecord()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v9

    .line 749
    .local v9, "first":Landroidx/compose/runtime/snapshots/StateRecord;
    move/from16 v10, p1

    invoke-static {v9, v10, v3}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$readable(Landroidx/compose/runtime/snapshots/StateRecord;ILandroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v11

    if-nez v11, :cond_0

    goto/16 :goto_6

    .line 750
    .local v11, "current":Landroidx/compose/runtime/snapshots/StateRecord;
    :cond_0
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v12

    invoke-static {v9, v12, v4}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$readable(Landroidx/compose/runtime/snapshots/StateRecord;ILandroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v12

    if-nez v12, :cond_1

    goto/16 :goto_6

    .line 751
    .local v12, "previous":Landroidx/compose/runtime/snapshots/StateRecord;
    :cond_1
    invoke-static {v11, v12}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v13

    if-nez v13, :cond_c

    .line 752
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v13

    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v14

    invoke-static {v9, v13, v14}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$readable(Landroidx/compose/runtime/snapshots/StateRecord;ILandroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v13

    if-eqz v13, :cond_b

    .line 753
    .local v13, "applied":Landroidx/compose/runtime/snapshots/StateRecord;
    if-nez v2, :cond_2

    const/4 v14, 0x0

    goto :goto_1

    :cond_2
    invoke-interface {v2, v11}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v14

    check-cast v14, Landroidx/compose/runtime/snapshots/StateRecord;

    :goto_1
    if-nez v14, :cond_3

    move-object v14, v1

    check-cast v14, Landroidx/compose/runtime/snapshots/MutableSnapshot;

    .local v14, "$this$innerApply_u24lambda_u2d9":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    const/4 v15, 0x0

    .line 754
    .local v15, "$i$a$-run-MutableSnapshot$innerApply$merged$1":I
    invoke-interface {v0, v12, v11, v13}, Landroidx/compose/runtime/snapshots/StateObject;->mergeRecords(Landroidx/compose/runtime/snapshots/StateRecord;Landroidx/compose/runtime/snapshots/StateRecord;Landroidx/compose/runtime/snapshots/StateRecord;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v14

    .line 753
    .end local v14    # "$this$innerApply_u24lambda_u2d9":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .end local v15    # "$i$a$-run-MutableSnapshot$innerApply$merged$1":I
    nop

    .line 756
    .local v14, "merged":Landroidx/compose/runtime/snapshots/StateRecord;
    :cond_3
    nop

    .line 757
    if-nez v14, :cond_4

    new-instance v7, Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Failure;

    move-object v15, v1

    check-cast v15, Landroidx/compose/runtime/snapshots/Snapshot;

    invoke-direct {v7, v15}, Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Failure;-><init>(Landroidx/compose/runtime/snapshots/Snapshot;)V

    check-cast v7, Landroidx/compose/runtime/snapshots/SnapshotApplyResult;

    return-object v7

    .line 758
    :cond_4
    invoke-static {v14, v13}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v15

    if-eqz v15, :cond_5

    goto/16 :goto_6

    .line 762
    :cond_5
    invoke-static {v14, v11}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v15

    if-eqz v15, :cond_8

    .line 764
    if-nez v6, :cond_6

    new-instance v15, Ljava/util/ArrayList;

    invoke-direct {v15}, Ljava/util/ArrayList;-><init>()V

    check-cast v15, Ljava/util/List;

    move-object/from16 v16, v15

    .local v16, "it":Ljava/util/List;
    const/16 v17, 0x0

    .line 765
    .local v17, "$i$a$-also-MutableSnapshot$innerApply$1":I
    move-object/from16 v6, v16

    .line 766
    nop

    .line 764
    .end local v16    # "it":Ljava/util/List;
    .end local v17    # "$i$a$-also-MutableSnapshot$innerApply$1":I
    move-object/from16 v22, v15

    move-object v15, v6

    move-object/from16 v6, v22

    goto :goto_2

    :cond_6
    move-object v15, v6

    .line 767
    .end local v6    # "mergedRecords":Ljava/lang/Object;
    .local v15, "mergedRecords":Ljava/lang/Object;
    :goto_2
    invoke-virtual {v11}, Landroidx/compose/runtime/snapshots/StateRecord;->create()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v1

    invoke-static {v0, v1}, Lkotlin/TuplesKt;->to(Ljava/lang/Object;Ljava/lang/Object;)Lkotlin/Pair;

    move-result-object v1

    invoke-interface {v6, v1}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    .line 771
    if-nez v8, :cond_7

    new-instance v1, Ljava/util/ArrayList;

    invoke-direct {v1}, Ljava/util/ArrayList;-><init>()V

    check-cast v1, Ljava/util/List;

    move-object v6, v1

    .local v6, "it":Ljava/util/List;
    const/16 v16, 0x0

    .line 772
    .local v16, "$i$a$-also-MutableSnapshot$innerApply$2":I
    move-object v8, v6

    .line 773
    nop

    .line 771
    .end local v6    # "it":Ljava/util/List;
    .end local v16    # "$i$a$-also-MutableSnapshot$innerApply$2":I
    move-object/from16 v22, v8

    move-object v8, v1

    move-object/from16 v1, v22

    goto :goto_3

    :cond_7
    move-object v1, v8

    .line 774
    .end local v8    # "statesToRemove":Ljava/lang/Object;
    .local v1, "statesToRemove":Ljava/lang/Object;
    :goto_3
    invoke-interface {v8, v0}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    move-object v8, v1

    move-object v6, v15

    move-object/from16 v1, p0

    goto/16 :goto_0

    .line 778
    .end local v1    # "statesToRemove":Ljava/lang/Object;
    .end local v15    # "mergedRecords":Ljava/lang/Object;
    .local v6, "mergedRecords":Ljava/lang/Object;
    .restart local v8    # "statesToRemove":Ljava/lang/Object;
    :cond_8
    if-nez v6, :cond_9

    new-instance v1, Ljava/util/ArrayList;

    invoke-direct {v1}, Ljava/util/ArrayList;-><init>()V

    check-cast v1, Ljava/util/List;

    move-object v15, v1

    .local v15, "it":Ljava/util/List;
    const/16 v16, 0x0

    .line 779
    .local v16, "$i$a$-also-MutableSnapshot$innerApply$3":I
    move-object v6, v15

    .line 780
    nop

    .line 778
    .end local v15    # "it":Ljava/util/List;
    .end local v16    # "$i$a$-also-MutableSnapshot$innerApply$3":I
    move-object/from16 v22, v6

    move-object v6, v1

    move-object/from16 v1, v22

    goto :goto_4

    :cond_9
    move-object v1, v6

    .line 782
    .end local v6    # "mergedRecords":Ljava/lang/Object;
    .local v1, "mergedRecords":Ljava/lang/Object;
    :goto_4
    invoke-static {v14, v12}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result v15

    if-nez v15, :cond_a

    invoke-static {v0, v14}, Lkotlin/TuplesKt;->to(Ljava/lang/Object;Ljava/lang/Object;)Lkotlin/Pair;

    move-result-object v15

    goto :goto_5

    .line 783
    :cond_a
    invoke-virtual {v12}, Landroidx/compose/runtime/snapshots/StateRecord;->create()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v15

    invoke-static {v0, v15}, Lkotlin/TuplesKt;->to(Ljava/lang/Object;Ljava/lang/Object;)Lkotlin/Pair;

    move-result-object v15

    .line 781
    :goto_5
    invoke-interface {v6, v15}, Ljava/util/List;->add(Ljava/lang/Object;)Z

    move-object v6, v1

    move-object/from16 v1, p0

    goto/16 :goto_0

    .line 752
    .end local v1    # "mergedRecords":Ljava/lang/Object;
    .end local v13    # "applied":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v14    # "merged":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v6    # "mergedRecords":Ljava/lang/Object;
    :cond_b
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$readError()Ljava/lang/Void;

    new-instance v1, Lkotlin/KotlinNothingValueException;

    invoke-direct {v1}, Lkotlin/KotlinNothingValueException;-><init>()V

    throw v1

    .line 745
    .end local v0    # "state":Landroidx/compose/runtime/snapshots/StateObject;
    .end local v9    # "first":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v11    # "current":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v12    # "previous":Landroidx/compose/runtime/snapshots/StateRecord;
    :cond_c
    :goto_6
    move-object/from16 v1, p0

    goto/16 :goto_0

    .line 790
    :cond_d
    move/from16 v10, p1

    if-nez v6, :cond_e

    goto :goto_9

    :cond_e
    move-object v1, v6

    .local v1, "it":Ljava/util/List;
    const/4 v7, 0x0

    .line 792
    .local v7, "$i$a$-let-MutableSnapshot$innerApply$4":I
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->advance$runtime_release()V

    .line 795
    move-object v9, v1

    .local v9, "$this$fastForEach$iv":Ljava/util/List;
    const/4 v11, 0x0

    .line 1818
    .local v11, "$i$f$fastForEach":I
    nop

    .line 1819
    invoke-interface {v9}, Ljava/util/List;->size()I

    move-result v0

    add-int/lit8 v0, v0, -0x1

    if-ltz v0, :cond_10

    const/4 v12, 0x0

    :goto_7
    move v13, v12

    .local v13, "index$iv":I
    add-int/lit8 v12, v12, 0x1

    .line 1820
    invoke-interface {v9, v13}, Ljava/util/List;->get(I)Ljava/lang/Object;

    move-result-object v14

    .line 1821
    .local v14, "item$iv":Ljava/lang/Object;
    move-object v15, v14

    check-cast v15, Lkotlin/Pair;

    .local v15, "merged":Lkotlin/Pair;
    const/16 v16, 0x0

    .line 796
    .local v16, "$i$a$-fastForEach-MutableSnapshot$innerApply$4$1":I
    invoke-virtual {v15}, Lkotlin/Pair;->component1()Ljava/lang/Object;

    move-result-object v17

    move-object/from16 v18, v1

    .end local v1    # "it":Ljava/util/List;
    .local v18, "it":Ljava/util/List;
    move-object/from16 v1, v17

    check-cast v1, Landroidx/compose/runtime/snapshots/StateObject;

    .local v1, "state":Landroidx/compose/runtime/snapshots/StateObject;
    invoke-virtual {v15}, Lkotlin/Pair;->component2()Ljava/lang/Object;

    move-result-object v17

    move-object/from16 v2, v17

    check-cast v2, Landroidx/compose/runtime/snapshots/StateRecord;

    .line 797
    .local v2, "stateRecord":Landroidx/compose/runtime/snapshots/StateRecord;
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v3

    invoke-virtual {v2, v3}, Landroidx/compose/runtime/snapshots/StateRecord;->setSnapshotId$runtime_release(I)V

    .line 798
    const/4 v3, 0x0

    .line 1822
    .local v3, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v17

    .local v17, "lock$iv$iv":Ljava/lang/Object;
    const/16 v19, 0x0

    .line 1823
    .local v19, "$i$f$synchronized":I
    monitor-enter v17

    const/16 v20, 0x0

    .line 799
    .local v20, "$i$a$-sync-MutableSnapshot$innerApply$4$1$1":I
    move/from16 v21, v3

    .end local v3    # "$i$f$sync":I
    .local v21, "$i$f$sync":I
    :try_start_0
    invoke-interface {v1}, Landroidx/compose/runtime/snapshots/StateObject;->getFirstStateRecord()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v3

    invoke-virtual {v2, v3}, Landroidx/compose/runtime/snapshots/StateRecord;->setNext$runtime_release(Landroidx/compose/runtime/snapshots/StateRecord;)V

    .line 800
    invoke-interface {v1, v2}, Landroidx/compose/runtime/snapshots/StateObject;->prependStateRecord(Landroidx/compose/runtime/snapshots/StateRecord;)V

    .line 801
    nop

    .end local v20    # "$i$a$-sync-MutableSnapshot$innerApply$4$1$1":I
    sget-object v3, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit v17

    .line 1823
    nop

    .line 1822
    .end local v17    # "lock$iv$iv":Ljava/lang/Object;
    .end local v19    # "$i$f$synchronized":I
    nop

    .line 802
    .end local v21    # "$i$f$sync":I
    nop

    .line 1819
    .end local v1    # "state":Landroidx/compose/runtime/snapshots/StateObject;
    .end local v2    # "stateRecord":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v14    # "item$iv":Ljava/lang/Object;
    .end local v15    # "merged":Lkotlin/Pair;
    .end local v16    # "$i$a$-fastForEach-MutableSnapshot$innerApply$4$1":I
    if-le v12, v0, :cond_f

    goto :goto_8

    :cond_f
    move-object/from16 v2, p2

    move-object/from16 v3, p3

    move-object/from16 v1, v18

    goto :goto_7

    .line 801
    .restart local v1    # "state":Landroidx/compose/runtime/snapshots/StateObject;
    .restart local v2    # "stateRecord":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v14    # "item$iv":Ljava/lang/Object;
    .restart local v15    # "merged":Lkotlin/Pair;
    .restart local v16    # "$i$a$-fastForEach-MutableSnapshot$innerApply$4$1":I
    .restart local v17    # "lock$iv$iv":Ljava/lang/Object;
    .restart local v19    # "$i$f$synchronized":I
    .restart local v21    # "$i$f$sync":I
    :catchall_0
    move-exception v0

    monitor-exit v17

    throw v0

    .line 1819
    .end local v2    # "stateRecord":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v13    # "index$iv":I
    .end local v14    # "item$iv":Ljava/lang/Object;
    .end local v15    # "merged":Lkotlin/Pair;
    .end local v16    # "$i$a$-fastForEach-MutableSnapshot$innerApply$4$1":I
    .end local v17    # "lock$iv$iv":Ljava/lang/Object;
    .end local v18    # "it":Ljava/util/List;
    .end local v19    # "$i$f$synchronized":I
    .end local v21    # "$i$f$sync":I
    .local v1, "it":Ljava/util/List;
    :cond_10
    move-object/from16 v18, v1

    .line 1824
    .end local v1    # "it":Ljava/util/List;
    .restart local v18    # "it":Ljava/util/List;
    :goto_8
    nop

    .line 803
    .end local v9    # "$this$fastForEach$iv":Ljava/util/List;
    .end local v11    # "$i$f$fastForEach":I
    nop

    .line 790
    .end local v7    # "$i$a$-let-MutableSnapshot$innerApply$4":I
    .end local v18    # "it":Ljava/util/List;
    nop

    .line 805
    :goto_9
    if-nez v8, :cond_11

    goto :goto_a

    :cond_11
    move-object v0, v8

    .local v0, "it":Ljava/util/List;
    const/4 v1, 0x0

    .line 807
    .local v1, "$i$a$-let-MutableSnapshot$innerApply$5":I
    move-object v2, v0

    check-cast v2, Ljava/util/Collection;

    invoke-interface {v5, v2}, Ljava/util/Set;->removeAll(Ljava/util/Collection;)Z

    .line 805
    .end local v0    # "it":Ljava/util/List;
    .end local v1    # "$i$a$-let-MutableSnapshot$innerApply$5":I
    nop

    .line 810
    :goto_a
    sget-object v0, Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Success;->INSTANCE:Landroidx/compose/runtime/snapshots/SnapshotApplyResult$Success;

    check-cast v0, Landroidx/compose/runtime/snapshots/SnapshotApplyResult;

    return-object v0
.end method

.method public nestedActivated$runtime_release(Landroidx/compose/runtime/snapshots/Snapshot;)V
    .locals 1
    .param p1, "snapshot"    # Landroidx/compose/runtime/snapshots/Snapshot;

    const-string v0, "snapshot"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 669
    iget v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->snapshots:I

    add-int/lit8 v0, v0, 0x1

    iput v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->snapshots:I

    return-void
.end method

.method public nestedDeactivated$runtime_release(Landroidx/compose/runtime/snapshots/Snapshot;)V
    .locals 2
    .param p1, "snapshot"    # Landroidx/compose/runtime/snapshots/Snapshot;

    const-string v0, "snapshot"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 672
    iget v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->snapshots:I

    if-lez v0, :cond_0

    const/4 v1, 0x1

    goto :goto_0

    :cond_0
    const/4 v1, 0x0

    :goto_0
    if-eqz v1, :cond_2

    .line 673
    add-int/lit8 v0, v0, -0x1

    iput v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->snapshots:I

    if-nez v0, :cond_1

    .line 674
    iget-boolean v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->applied:Z

    if-nez v0, :cond_1

    .line 675
    invoke-direct {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->abandon()V

    .line 678
    :cond_1
    return-void

    .line 672
    :cond_2
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Failed requirement."

    invoke-virtual {v1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    check-cast v0, Ljava/lang/Throwable;

    throw v0
.end method

.method public notifyObjectsInitialized$runtime_release()V
    .locals 1

    .line 681
    iget-boolean v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->applied:Z

    if-nez v0, :cond_1

    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getDisposed$runtime_release()Z

    move-result v0

    if-eqz v0, :cond_0

    goto :goto_0

    .line 682
    :cond_0
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->advance$runtime_release()V

    .line 683
    return-void

    .line 681
    :cond_1
    :goto_0
    return-void
.end method

.method public recordModified$runtime_release(Landroidx/compose/runtime/snapshots/StateObject;)V
    .locals 4
    .param p1, "state"    # Landroidx/compose/runtime/snapshots/StateObject;

    const-string v0, "state"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 844
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getModified$runtime_release()Ljava/util/Set;

    move-result-object v0

    if-nez v0, :cond_0

    new-instance v0, Ljava/util/HashSet;

    invoke-direct {v0}, Ljava/util/HashSet;-><init>()V

    move-object v1, v0

    .line 1846
    .local v1, "it":Ljava/util/HashSet;
    const/4 v2, 0x0

    .line 844
    .local v2, "$i$a$-also-MutableSnapshot$recordModified$1":I
    move-object v3, v1

    check-cast v3, Ljava/util/Set;

    invoke-virtual {p0, v3}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setModified(Ljava/util/Set;)V

    .end local v1    # "it":Ljava/util/HashSet;
    .end local v2    # "$i$a$-also-MutableSnapshot$recordModified$1":I
    check-cast v0, Ljava/util/Set;

    :cond_0
    invoke-interface {v0, p1}, Ljava/util/Set;->add(Ljava/lang/Object;)Z

    .line 845
    return-void
.end method

.method public final recordPrevious$runtime_release(I)V
    .locals 5
    .param p1, "id"    # I

    .line 832
    const/4 v0, 0x0

    .line 1842
    .local v0, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v1

    .local v1, "lock$iv$iv":Ljava/lang/Object;
    const/4 v2, 0x0

    .line 1843
    .local v2, "$i$f$synchronized":I
    monitor-enter v1

    const/4 v3, 0x0

    .line 833
    .local v3, "$i$a$-sync-MutableSnapshot$recordPrevious$1":I
    :try_start_0
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getPreviousIds$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-virtual {v4, p1}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-virtual {p0, v4}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setPreviousIds$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 834
    nop

    .end local v3    # "$i$a$-sync-MutableSnapshot$recordPrevious$1":I
    sget-object v3, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit v1

    .line 1843
    nop

    .line 1842
    .end local v1    # "lock$iv$iv":Ljava/lang/Object;
    .end local v2    # "$i$f$synchronized":I
    nop

    .line 835
    .end local v0    # "$i$f$sync":I
    return-void

    .line 834
    .restart local v0    # "$i$f$sync":I
    .restart local v1    # "lock$iv$iv":Ljava/lang/Object;
    .restart local v2    # "$i$f$synchronized":I
    :catchall_0
    move-exception v3

    monitor-exit v1

    throw v3
.end method

.method public final recordPreviousList$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V
    .locals 5
    .param p1, "snapshots"    # Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    const-string v0, "snapshots"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 838
    const/4 v0, 0x0

    .line 1844
    .local v0, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v1

    .local v1, "lock$iv$iv":Ljava/lang/Object;
    const/4 v2, 0x0

    .line 1845
    .local v2, "$i$f$synchronized":I
    monitor-enter v1

    const/4 v3, 0x0

    .line 839
    .local v3, "$i$a$-sync-MutableSnapshot$recordPreviousList$1":I
    :try_start_0
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getPreviousIds$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-virtual {v4, p1}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->or(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v4

    invoke-virtual {p0, v4}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setPreviousIds$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 840
    nop

    .end local v3    # "$i$a$-sync-MutableSnapshot$recordPreviousList$1":I
    sget-object v3, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit v1

    .line 1845
    nop

    .line 1844
    .end local v1    # "lock$iv$iv":Ljava/lang/Object;
    .end local v2    # "$i$f$synchronized":I
    nop

    .line 841
    .end local v0    # "$i$f$sync":I
    return-void

    .line 840
    .restart local v0    # "$i$f$sync":I
    .restart local v1    # "lock$iv$iv":Ljava/lang/Object;
    .restart local v2    # "$i$f$synchronized":I
    :catchall_0
    move-exception v3

    monitor-exit v1

    throw v3
.end method

.method public final setApplied$runtime_release(Z)V
    .locals 0
    .param p1, "<set-?>"    # Z

    .line 864
    iput-boolean p1, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->applied:Z

    return-void
.end method

.method public setModified(Ljava/util/Set;)V
    .locals 0
    .param p1, "<set-?>"    # Ljava/util/Set;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/Set<",
            "Landroidx/compose/runtime/snapshots/StateObject;",
            ">;)V"
        }
    .end annotation

    .line 847
    iput-object p1, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->modified:Ljava/util/Set;

    return-void
.end method

.method public final setPreviousIds$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V
    .locals 1
    .param p1, "<set-?>"    # Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    const-string v0, "<set-?>"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 853
    iput-object p1, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->previousIds:Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    return-void
.end method

.method public takeNestedMutableSnapshot(Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;)Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .locals 18
    .param p1, "readObserver"    # Lkotlin/jvm/functions/Function1;
    .param p2, "writeObserver"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;)",
            "Landroidx/compose/runtime/snapshots/MutableSnapshot;"
        }
    .end annotation

    .line 533
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->validateNotDisposed$runtime_release()V

    .line 534
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->validateNotApplied$runtime_release()V

    .line 535
    move-object/from16 v1, p0

    .local v1, "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    const/4 v2, 0x0

    .line 1768
    .local v2, "$i$f$advance$runtime_release":I
    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v0

    invoke-virtual {v1, v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->recordPrevious$runtime_release(I)V

    .line 1769
    const/4 v3, 0x0

    .line 536
    .local v3, "$i$a$-advance$runtime_release-MutableSnapshot$takeNestedMutableSnapshot$1":I
    const/4 v4, 0x0

    .line 1770
    .local v4, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v5

    .local v5, "lock$iv$iv":Ljava/lang/Object;
    const/4 v6, 0x0

    .line 1771
    .local v6, "$i$f$synchronized":I
    monitor-enter v5

    const/4 v0, 0x0

    .line 537
    .local v0, "$i$a$-sync-MutableSnapshot$takeNestedMutableSnapshot$1$1":I
    :try_start_0
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getNextSnapshotId$p()I

    move-result v7

    add-int/lit8 v8, v7, 0x1

    invoke-static {v8}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setNextSnapshotId$p(I)V

    .line 538
    .local v7, "newId":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v8

    invoke-virtual {v8, v7}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v8

    invoke-static {v8}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setOpenSnapshots$p(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 539
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v8

    .line 540
    .local v8, "invalid":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    invoke-virtual {v8, v7}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_2

    move-object/from16 v15, p0

    :try_start_1
    invoke-virtual {v15, v9}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setInvalid$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 541
    new-instance v16, Landroidx/compose/runtime/snapshots/NestedMutableSnapshot;

    .line 542
    nop

    .line 543
    nop

    .line 544
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getReadObserver$runtime_release()Lkotlin/jvm/functions/Function1;

    move-result-object v9

    move-object/from16 v14, p1

    invoke-static {v14, v9}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$mergedReadObserver(Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;)Lkotlin/jvm/functions/Function1;

    move-result-object v12

    .line 545
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getWriteObserver$runtime_release()Lkotlin/jvm/functions/Function1;

    move-result-object v9

    move-object/from16 v13, p2

    invoke-static {v13, v9}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$mergedWriteObserver(Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;)Lkotlin/jvm/functions/Function1;

    move-result-object v17

    .line 546
    nop

    .line 541
    move-object/from16 v9, v16

    move v10, v7

    move-object v11, v8

    move-object/from16 v13, v17

    move-object/from16 v14, p0

    invoke-direct/range {v9 .. v14}, Landroidx/compose/runtime/snapshots/NestedMutableSnapshot;-><init>(ILandroidx/compose/runtime/snapshots/SnapshotIdSet;Lkotlin/jvm/functions/Function1;Lkotlin/jvm/functions/Function1;Landroidx/compose/runtime/snapshots/MutableSnapshot;)V
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_1

    .end local v0    # "$i$a$-sync-MutableSnapshot$takeNestedMutableSnapshot$1$1":I
    .end local v7    # "newId":I
    .end local v8    # "invalid":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    monitor-exit v5

    .line 1771
    nop

    .line 1770
    .end local v5    # "lock$iv$iv":Ljava/lang/Object;
    .end local v6    # "$i$f$synchronized":I
    nop

    .line 536
    .end local v4    # "$i$f$sync":I
    nop

    .end local v3    # "$i$a$-advance$runtime_release-MutableSnapshot$takeNestedMutableSnapshot$1":I
    move-object/from16 v3, v16

    .local v3, "it$iv":Ljava/lang/Object;
    const/4 v4, 0x0

    .line 1772
    .local v4, "$i$a$-also-MutableSnapshot$advance$1$iv":I
    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v5

    .line 1773
    .local v5, "previousId$iv":I
    const/4 v6, 0x0

    .line 1774
    .local v6, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v7

    .local v7, "lock$iv$iv$iv":Ljava/lang/Object;
    const/4 v8, 0x0

    .line 1775
    .local v8, "$i$f$synchronized":I
    monitor-enter v7

    const/4 v0, 0x0

    .line 1776
    .local v0, "$i$a$-sync-MutableSnapshot$advance$1$1$iv":I
    :try_start_2
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getNextSnapshotId$p()I

    move-result v9

    add-int/lit8 v10, v9, 0x1

    invoke-static {v10}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setNextSnapshotId$p(I)V

    invoke-virtual {v1, v9}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setId$runtime_release(I)V

    .line 1777
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9

    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v10

    invoke-virtual {v9, v10}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9

    invoke-static {v9}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setOpenSnapshots$p(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 1778
    nop

    .end local v0    # "$i$a$-sync-MutableSnapshot$advance$1$1$iv":I
    sget-object v0, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_0

    monitor-exit v7

    .line 1775
    nop

    .line 1774
    .end local v7    # "lock$iv$iv$iv":Ljava/lang/Object;
    .end local v8    # "$i$f$synchronized":I
    nop

    .line 1779
    .end local v6    # "$i$f$sync":I
    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v0

    .line 1780
    .local v0, "currentInvalid$iv":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    add-int/lit8 v6, v5, 0x1

    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v7

    if-ge v6, v7, :cond_1

    :cond_0
    move v8, v6

    .local v8, "invalidId$iv":I
    add-int/lit8 v6, v6, 0x1

    .line 1781
    invoke-virtual {v0, v8}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v0

    .line 1780
    if-lt v6, v7, :cond_0

    .line 1783
    .end local v8    # "invalidId$iv":I
    :cond_1
    invoke-virtual {v1, v0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setInvalid$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 1784
    nop

    .line 1769
    .end local v0    # "currentInvalid$iv":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    .end local v3    # "it$iv":Ljava/lang/Object;
    .end local v4    # "$i$a$-also-MutableSnapshot$advance$1$iv":I
    .end local v5    # "previousId$iv":I
    nop

    .end local v1    # "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .end local v2    # "$i$f$advance$runtime_release":I
    check-cast v16, Landroidx/compose/runtime/snapshots/MutableSnapshot;

    .line 535
    return-object v16

    .line 1778
    .restart local v1    # "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .restart local v2    # "$i$f$advance$runtime_release":I
    .restart local v3    # "it$iv":Ljava/lang/Object;
    .restart local v4    # "$i$a$-also-MutableSnapshot$advance$1$iv":I
    .restart local v5    # "previousId$iv":I
    .restart local v6    # "$i$f$sync":I
    .restart local v7    # "lock$iv$iv$iv":Ljava/lang/Object;
    .local v8, "$i$f$synchronized":I
    :catchall_0
    move-exception v0

    monitor-exit v7

    throw v0

    .line 541
    .end local v7    # "lock$iv$iv$iv":Ljava/lang/Object;
    .end local v8    # "$i$f$synchronized":I
    .local v3, "$i$a$-advance$runtime_release-MutableSnapshot$takeNestedMutableSnapshot$1":I
    .local v4, "$i$f$sync":I
    .local v5, "lock$iv$iv":Ljava/lang/Object;
    .local v6, "$i$f$synchronized":I
    :catchall_1
    move-exception v0

    goto :goto_0

    :catchall_2
    move-exception v0

    move-object/from16 v15, p0

    :goto_0
    monitor-exit v5

    throw v0
.end method

.method public takeNestedSnapshot(Lkotlin/jvm/functions/Function1;)Landroidx/compose/runtime/snapshots/Snapshot;
    .locals 13
    .param p1, "readObserver"    # Lkotlin/jvm/functions/Function1;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lkotlin/jvm/functions/Function1<",
            "Ljava/lang/Object;",
            "Lkotlin/Unit;",
            ">;)",
            "Landroidx/compose/runtime/snapshots/Snapshot;"
        }
    .end annotation

    .line 648
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->validateNotDisposed$runtime_release()V

    .line 649
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->validateNotApplied$runtime_release()V

    .line 650
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v0

    .line 651
    .local v0, "previousId":I
    move-object v1, p0

    .local v1, "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    const/4 v2, 0x0

    .line 1799
    .local v2, "$i$f$advance$runtime_release":I
    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v3

    invoke-virtual {v1, v3}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->recordPrevious$runtime_release(I)V

    .line 1800
    const/4 v3, 0x0

    .line 652
    .local v3, "$i$a$-advance$runtime_release-MutableSnapshot$takeNestedSnapshot$1":I
    const/4 v4, 0x0

    .line 1801
    .local v4, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v5

    .local v5, "lock$iv$iv":Ljava/lang/Object;
    const/4 v6, 0x0

    .line 1802
    .local v6, "$i$f$synchronized":I
    monitor-enter v5

    const/4 v7, 0x0

    .line 653
    .local v7, "$i$a$-sync-MutableSnapshot$takeNestedSnapshot$1$1":I
    :try_start_0
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getNextSnapshotId$p()I

    move-result v8

    add-int/lit8 v9, v8, 0x1

    invoke-static {v9}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setNextSnapshotId$p(I)V

    .line 654
    .local v8, "readonlyId":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9

    invoke-virtual {v9, v8}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9

    invoke-static {v9}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setOpenSnapshots$p(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 655
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v9

    .line 656
    .local v9, "currentInvalid":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    add-int/lit8 v10, v0, 0x1

    if-ge v10, v8, :cond_1

    :cond_0
    move v11, v10

    .local v11, "invalidId":I
    add-int/lit8 v10, v10, 0x1

    .line 657
    invoke-virtual {v9, v11}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v12

    move-object v9, v12

    .line 656
    if-lt v10, v8, :cond_0

    .line 659
    .end local v11    # "invalidId":I
    :cond_1
    new-instance v10, Landroidx/compose/runtime/snapshots/NestedReadonlySnapshot;

    .line 660
    nop

    .line 661
    nop

    .line 662
    nop

    .line 663
    move-object v11, p0

    check-cast v11, Landroidx/compose/runtime/snapshots/Snapshot;

    .line 659
    invoke-direct {v10, v8, v9, p1, v11}, Landroidx/compose/runtime/snapshots/NestedReadonlySnapshot;-><init>(ILandroidx/compose/runtime/snapshots/SnapshotIdSet;Lkotlin/jvm/functions/Function1;Landroidx/compose/runtime/snapshots/Snapshot;)V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_1

    .end local v7    # "$i$a$-sync-MutableSnapshot$takeNestedSnapshot$1$1":I
    .end local v8    # "readonlyId":I
    .end local v9    # "currentInvalid":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    monitor-exit v5

    .line 1802
    nop

    .line 1801
    .end local v5    # "lock$iv$iv":Ljava/lang/Object;
    .end local v6    # "$i$f$synchronized":I
    nop

    .line 652
    .end local v4    # "$i$f$sync":I
    nop

    .end local v3    # "$i$a$-advance$runtime_release-MutableSnapshot$takeNestedSnapshot$1":I
    move-object v3, v10

    .local v3, "it$iv":Ljava/lang/Object;
    const/4 v4, 0x0

    .line 1803
    .local v4, "$i$a$-also-MutableSnapshot$advance$1$iv":I
    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v5

    .line 1804
    .local v5, "previousId$iv":I
    const/4 v6, 0x0

    .line 1805
    .local v6, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v7

    .local v7, "lock$iv$iv$iv":Ljava/lang/Object;
    const/4 v8, 0x0

    .line 1806
    .local v8, "$i$f$synchronized":I
    monitor-enter v7

    const/4 v9, 0x0

    .line 1807
    .local v9, "$i$a$-sync-MutableSnapshot$advance$1$1$iv":I
    :try_start_1
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getNextSnapshotId$p()I

    move-result v11

    add-int/lit8 v12, v11, 0x1

    invoke-static {v12}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setNextSnapshotId$p(I)V

    invoke-virtual {v1, v11}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setId$runtime_release(I)V

    .line 1808
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$getOpenSnapshots$p()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v11

    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v12

    invoke-virtual {v11, v12}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v11

    invoke-static {v11}, Landroidx/compose/runtime/snapshots/SnapshotKt;->access$setOpenSnapshots$p(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 1809
    nop

    .end local v9    # "$i$a$-sync-MutableSnapshot$advance$1$1$iv":I
    sget-object v9, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_0

    monitor-exit v7

    .line 1806
    nop

    .line 1805
    .end local v7    # "lock$iv$iv$iv":Ljava/lang/Object;
    .end local v8    # "$i$f$synchronized":I
    nop

    .line 1810
    .end local v6    # "$i$f$sync":I
    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getInvalid$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v6

    .line 1811
    .local v6, "currentInvalid$iv":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    add-int/lit8 v7, v5, 0x1

    invoke-virtual {v1}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->getId()I

    move-result v8

    if-ge v7, v8, :cond_3

    :cond_2
    move v9, v7

    .local v9, "invalidId$iv":I
    add-int/lit8 v7, v7, 0x1

    .line 1812
    invoke-virtual {v6, v9}, Landroidx/compose/runtime/snapshots/SnapshotIdSet;->set(I)Landroidx/compose/runtime/snapshots/SnapshotIdSet;

    move-result-object v6

    .line 1811
    if-lt v7, v8, :cond_2

    .line 1814
    .end local v9    # "invalidId$iv":I
    :cond_3
    invoke-virtual {v1, v6}, Landroidx/compose/runtime/snapshots/MutableSnapshot;->setInvalid$runtime_release(Landroidx/compose/runtime/snapshots/SnapshotIdSet;)V

    .line 1815
    nop

    .line 1800
    .end local v3    # "it$iv":Ljava/lang/Object;
    .end local v4    # "$i$a$-also-MutableSnapshot$advance$1$iv":I
    .end local v5    # "previousId$iv":I
    .end local v6    # "currentInvalid$iv":Landroidx/compose/runtime/snapshots/SnapshotIdSet;
    nop

    .end local v1    # "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .end local v2    # "$i$f$advance$runtime_release":I
    check-cast v10, Landroidx/compose/runtime/snapshots/Snapshot;

    .line 651
    return-object v10

    .line 1809
    .restart local v1    # "this_$iv":Landroidx/compose/runtime/snapshots/MutableSnapshot;
    .restart local v2    # "$i$f$advance$runtime_release":I
    .restart local v3    # "it$iv":Ljava/lang/Object;
    .restart local v4    # "$i$a$-also-MutableSnapshot$advance$1$iv":I
    .restart local v5    # "previousId$iv":I
    .local v6, "$i$f$sync":I
    .restart local v7    # "lock$iv$iv$iv":Ljava/lang/Object;
    .restart local v8    # "$i$f$synchronized":I
    :catchall_0
    move-exception v9

    monitor-exit v7

    throw v9

    .line 659
    .end local v7    # "lock$iv$iv$iv":Ljava/lang/Object;
    .end local v8    # "$i$f$synchronized":I
    .local v3, "$i$a$-advance$runtime_release-MutableSnapshot$takeNestedSnapshot$1":I
    .local v4, "$i$f$sync":I
    .local v5, "lock$iv$iv":Ljava/lang/Object;
    .local v6, "$i$f$synchronized":I
    :catchall_1
    move-exception v7

    monitor-exit v5

    throw v7
.end method

.method public final validateNotApplied$runtime_release()V
    .locals 2

    .line 693
    iget-boolean v0, p0, Landroidx/compose/runtime/snapshots/MutableSnapshot;->applied:Z

    xor-int/lit8 v0, v0, 0x1

    if-eqz v0, :cond_0

    .line 696
    return-void

    .line 693
    :cond_0
    const/4 v0, 0x0

    .line 694
    .local v0, "$i$a$-require-MutableSnapshot$validateNotApplied$1":I
    nop

    .line 693
    .end local v0    # "$i$a$-require-MutableSnapshot$validateNotApplied$1":I
    new-instance v0, Ljava/lang/IllegalArgumentException;

    const-string v1, "Unsupported operation on a snapshot that has been applied"

    invoke-virtual {v1}, Ljava/lang/Object;->toString()Ljava/lang/String;

    move-result-object v1

    invoke-direct {v0, v1}, Ljava/lang/IllegalArgumentException;-><init>(Ljava/lang/String;)V

    check-cast v0, Ljava/lang/Throwable;

    throw v0
.end method
