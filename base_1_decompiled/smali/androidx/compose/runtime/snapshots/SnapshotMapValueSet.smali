.class final Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;
.super Landroidx/compose/runtime/snapshots/SnapshotMapSet;
.source "SnapshotStateMap.kt"


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "<K:",
        "Ljava/lang/Object;",
        "V:",
        "Ljava/lang/Object;",
        ">",
        "Landroidx/compose/runtime/snapshots/SnapshotMapSet<",
        "TK;TV;TV;>;"
    }
.end annotation

.annotation system Ldalvik/annotation/SourceDebugExtension;
    value = "SMAP\nSnapshotStateMap.kt\nKotlin\n*S Kotlin\n*F\n+ 1 SnapshotStateMap.kt\nandroidx/compose/runtime/snapshots/SnapshotMapValueSet\n+ 2 SnapshotStateMap.kt\nandroidx/compose/runtime/snapshots/SnapshotStateMap\n+ 3 Snapshot.kt\nandroidx/compose/runtime/snapshots/SnapshotKt\n+ 4 ActualJvm.jvm.kt\nandroidx/compose/runtime/ActualJvm_jvmKt\n+ 5 _Collections.kt\nkotlin/collections/CollectionsKt___CollectionsKt\n*L\n1#1,300:1\n66#2,2:301\n101#2:303\n94#2:304\n102#2,2:306\n68#2,7:308\n104#2,2:315\n98#2:317\n106#2,2:324\n109#2,2:329\n75#2:331\n66#2,2:332\n101#2:334\n94#2:335\n102#2,2:337\n68#2,7:339\n104#2,2:346\n98#2:348\n106#2,2:355\n109#2,2:360\n75#2:362\n1766#3:305\n1673#3,2:318\n1371#3:320\n1675#3,2:322\n1677#3,3:326\n1766#3:336\n1673#3,2:349\n1371#3:351\n1675#3,2:353\n1677#3,3:357\n62#4:321\n62#4:352\n1720#5,3:363\n*S KotlinDebug\n*F\n+ 1 SnapshotStateMap.kt\nandroidx/compose/runtime/snapshots/SnapshotMapValueSet\n*L\n204#1:301,2\n204#1:303\n204#1:304\n204#1:306,2\n204#1:308,7\n204#1:315,2\n204#1:317\n204#1:324,2\n204#1:329,2\n204#1:331\n208#1:332,2\n208#1:334\n208#1:335\n208#1:337,2\n208#1:339,7\n208#1:346,2\n208#1:348\n208#1:355,2\n208#1:360,2\n208#1:362\n204#1:305\n204#1:318,2\n204#1:320\n204#1:322,2\n204#1:326,3\n208#1:336\n208#1:349,2\n208#1:351\n208#1:353,2\n208#1:357,3\n204#1:321\n208#1:352\n212#1:363,3\n*E\n"
.end annotation

.annotation runtime Lkotlin/Metadata;
    d1 = {
        "\u00004\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0010\u0001\n\u0002\u0008\u0004\n\u0002\u0010\u001e\n\u0000\n\u0002\u0010\u000b\n\u0002\u0008\u0003\n\u0002\u0018\u0002\n\u0002\u0008\u0004\u0008\u0002\u0018\u0000*\u0004\u0008\u0000\u0010\u0001*\u0004\u0008\u0001\u0010\u00022\u0014\u0012\u0004\u0012\u0002H\u0001\u0012\u0004\u0012\u0002H\u0002\u0012\u0004\u0012\u0002H\u00020\u0003B\u0019\u0012\u0012\u0010\u0004\u001a\u000e\u0012\u0004\u0012\u00028\u0000\u0012\u0004\u0012\u00028\u00010\u0005\u00a2\u0006\u0002\u0010\u0006J\u0015\u0010\u0007\u001a\u00020\u00082\u0006\u0010\t\u001a\u00028\u0001H\u0016\u00a2\u0006\u0002\u0010\nJ\u0016\u0010\u000b\u001a\u00020\u00082\u000c\u0010\u000c\u001a\u0008\u0012\u0004\u0012\u00028\u00010\rH\u0016J\u0016\u0010\u000e\u001a\u00020\u000f2\u0006\u0010\t\u001a\u00028\u0001H\u0096\u0002\u00a2\u0006\u0002\u0010\u0010J\u0016\u0010\u0011\u001a\u00020\u000f2\u000c\u0010\u000c\u001a\u0008\u0012\u0004\u0012\u00028\u00010\rH\u0016J\u0015\u0010\u0012\u001a\u000e\u0012\u0004\u0012\u00028\u0000\u0012\u0004\u0012\u00028\u00010\u0013H\u0096\u0002J\u0015\u0010\u0014\u001a\u00020\u000f2\u0006\u0010\t\u001a\u00028\u0001H\u0016\u00a2\u0006\u0002\u0010\u0010J\u0016\u0010\u0015\u001a\u00020\u000f2\u000c\u0010\u000c\u001a\u0008\u0012\u0004\u0012\u00028\u00010\rH\u0016J\u0016\u0010\u0016\u001a\u00020\u000f2\u000c\u0010\u000c\u001a\u0008\u0012\u0004\u0012\u00028\u00010\rH\u0016\u00a8\u0006\u0017"
    }
    d2 = {
        "Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;",
        "K",
        "V",
        "Landroidx/compose/runtime/snapshots/SnapshotMapSet;",
        "map",
        "Landroidx/compose/runtime/snapshots/SnapshotStateMap;",
        "(Landroidx/compose/runtime/snapshots/SnapshotStateMap;)V",
        "add",
        "",
        "element",
        "(Ljava/lang/Object;)Ljava/lang/Void;",
        "addAll",
        "elements",
        "",
        "contains",
        "",
        "(Ljava/lang/Object;)Z",
        "containsAll",
        "iterator",
        "Landroidx/compose/runtime/snapshots/StateMapMutableValuesIterator;",
        "remove",
        "removeAll",
        "retainAll",
        "runtime_release"
    }
    k = 0x1
    mv = {
        0x1,
        0x5,
        0x1
    }
    xi = 0x30
.end annotation


# direct methods
.method public constructor <init>(Landroidx/compose/runtime/snapshots/SnapshotStateMap;)V
    .locals 1
    .param p1, "map"    # Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Landroidx/compose/runtime/snapshots/SnapshotStateMap<",
            "TK;TV;>;)V"
        }
    .end annotation

    const-string v0, "map"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 196
    invoke-direct {p0, p1}, Landroidx/compose/runtime/snapshots/SnapshotMapSet;-><init>(Landroidx/compose/runtime/snapshots/SnapshotStateMap;)V

    .line 194
    return-void
.end method


# virtual methods
.method public add(Ljava/lang/Object;)Ljava/lang/Void;
    .locals 1
    .param p1, "element"    # Ljava/lang/Object;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(TV;)",
            "Ljava/lang/Void;"
        }
    .end annotation

    .line 197
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotStateMapKt;->unsupported()Ljava/lang/Void;

    new-instance v0, Lkotlin/KotlinNothingValueException;

    invoke-direct {v0}, Lkotlin/KotlinNothingValueException;-><init>()V

    throw v0
.end method

.method public bridge synthetic add(Ljava/lang/Object;)Z
    .locals 1
    .param p1, "element"    # Ljava/lang/Object;

    .line 194
    invoke-virtual {p0, p1}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->add(Ljava/lang/Object;)Ljava/lang/Void;

    move-result-object v0

    check-cast v0, Ljava/lang/Boolean;

    invoke-virtual {v0}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v0

    return v0
.end method

.method public addAll(Ljava/util/Collection;)Ljava/lang/Void;
    .locals 1
    .param p1, "elements"    # Ljava/util/Collection;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/Collection<",
            "+TV;>;)",
            "Ljava/lang/Void;"
        }
    .end annotation

    const-string v0, "elements"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 198
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotStateMapKt;->unsupported()Ljava/lang/Void;

    new-instance v0, Lkotlin/KotlinNothingValueException;

    invoke-direct {v0}, Lkotlin/KotlinNothingValueException;-><init>()V

    throw v0
.end method

.method public bridge synthetic addAll(Ljava/util/Collection;)Z
    .locals 1
    .param p1, "elements"    # Ljava/util/Collection;

    .line 194
    invoke-virtual {p0, p1}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->addAll(Ljava/util/Collection;)Ljava/lang/Void;

    move-result-object v0

    check-cast v0, Ljava/lang/Boolean;

    invoke-virtual {v0}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v0

    return v0
.end method

.method public contains(Ljava/lang/Object;)Z
    .locals 1
    .param p1, "element"    # Ljava/lang/Object;

    .line 210
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->getMap()Landroidx/compose/runtime/snapshots/SnapshotStateMap;

    move-result-object v0

    invoke-virtual {v0, p1}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->containsValue(Ljava/lang/Object;)Z

    move-result v0

    return v0
.end method

.method public containsAll(Ljava/util/Collection;)Z
    .locals 8
    .param p1, "elements"    # Ljava/util/Collection;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/Collection<",
            "+",
            "Ljava/lang/Object;",
            ">;)Z"
        }
    .end annotation

    const-string v0, "elements"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 212
    move-object v0, p1

    check-cast v0, Ljava/lang/Iterable;

    .local v0, "$this$all$iv":Ljava/lang/Iterable;
    const/4 v1, 0x0

    .line 363
    .local v1, "$i$f$all":I
    move-object v2, v0

    check-cast v2, Ljava/util/Collection;

    invoke-interface {v2}, Ljava/util/Collection;->isEmpty()Z

    move-result v2

    const/4 v3, 0x1

    if-eqz v2, :cond_0

    goto :goto_0

    .line 364
    :cond_0
    invoke-interface {v0}, Ljava/lang/Iterable;->iterator()Ljava/util/Iterator;

    move-result-object v2

    :cond_1
    invoke-interface {v2}, Ljava/util/Iterator;->hasNext()Z

    move-result v4

    if-eqz v4, :cond_2

    invoke-interface {v2}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v4

    .local v4, "element$iv":Ljava/lang/Object;
    move-object v5, v4

    .local v5, "it":Ljava/lang/Object;
    const/4 v6, 0x0

    .line 212
    .local v6, "$i$a$-all-SnapshotMapValueSet$containsAll$1":I
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->getMap()Landroidx/compose/runtime/snapshots/SnapshotStateMap;

    move-result-object v7

    invoke-virtual {v7, v5}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->containsValue(Ljava/lang/Object;)Z

    move-result v5

    .end local v5    # "it":Ljava/lang/Object;
    .end local v6    # "$i$a$-all-SnapshotMapValueSet$containsAll$1":I
    if-nez v5, :cond_1

    const/4 v3, 0x0

    goto :goto_0

    .line 365
    .end local v4    # "element$iv":Ljava/lang/Object;
    :cond_2
    nop

    .line 212
    .end local v0    # "$this$all$iv":Ljava/lang/Iterable;
    .end local v1    # "$i$f$all":I
    :goto_0
    return v3
.end method

.method public iterator()Landroidx/compose/runtime/snapshots/StateMapMutableValuesIterator;
    .locals 3
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()",
            "Landroidx/compose/runtime/snapshots/StateMapMutableValuesIterator<",
            "TK;TV;>;"
        }
    .end annotation

    .line 200
    new-instance v0, Landroidx/compose/runtime/snapshots/StateMapMutableValuesIterator;

    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->getMap()Landroidx/compose/runtime/snapshots/SnapshotStateMap;

    move-result-object v1

    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->getMap()Landroidx/compose/runtime/snapshots/SnapshotStateMap;

    move-result-object v2

    invoke-virtual {v2}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->getReadable$runtime_release()Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    move-result-object v2

    invoke-virtual {v2}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->getMap$runtime_release()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;

    move-result-object v2

    invoke-interface {v2}, Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;->entrySet()Ljava/util/Set;

    move-result-object v2

    check-cast v2, Landroidx/compose/runtime/external/kotlinx/collections/immutable/ImmutableSet;

    invoke-interface {v2}, Landroidx/compose/runtime/external/kotlinx/collections/immutable/ImmutableSet;->iterator()Ljava/util/Iterator;

    move-result-object v2

    invoke-direct {v0, v1, v2}, Landroidx/compose/runtime/snapshots/StateMapMutableValuesIterator;-><init>(Landroidx/compose/runtime/snapshots/SnapshotStateMap;Ljava/util/Iterator;)V

    return-object v0
.end method

.method public bridge synthetic iterator()Ljava/util/Iterator;
    .locals 1

    .line 194
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->iterator()Landroidx/compose/runtime/snapshots/StateMapMutableValuesIterator;

    move-result-object v0

    check-cast v0, Ljava/util/Iterator;

    return-object v0
.end method

.method public remove(Ljava/lang/Object;)Z
    .locals 1
    .param p1, "element"    # Ljava/lang/Object;

    .line 201
    invoke-virtual {p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->getMap()Landroidx/compose/runtime/snapshots/SnapshotStateMap;

    move-result-object v0

    invoke-virtual {v0, p1}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->removeValue$runtime_release(Ljava/lang/Object;)Z

    move-result v0

    return v0
.end method

.method public removeAll(Ljava/util/Collection;)Z
    .locals 28
    .param p1, "elements"    # Ljava/util/Collection;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/Collection<",
            "+",
            "Ljava/lang/Object;",
            ">;)Z"
        }
    .end annotation

    move-object/from16 v1, p1

    const-string v0, "elements"

    invoke-static {v1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 203
    move-object v0, v1

    check-cast v0, Ljava/lang/Iterable;

    invoke-static {v0}, Lkotlin/collections/CollectionsKt;->toSet(Ljava/lang/Iterable;)Ljava/util/Set;

    move-result-object v2

    .line 204
    .local v2, "set":Ljava/util/Set;
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->getMap()Landroidx/compose/runtime/snapshots/SnapshotStateMap;

    move-result-object v3

    .local v3, "this_$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/4 v4, 0x0

    .line 301
    .local v4, "$i$f$removeIf$runtime_release":I
    const/4 v0, 0x0

    .line 302
    .local v0, "removed$iv":Z
    move-object v5, v3

    .local v5, "this_$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/4 v6, 0x0

    .line 303
    .local v6, "$i$f$mutate":I
    move-object v7, v5

    .local v7, "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/4 v8, 0x0

    .line 304
    .local v8, "$i$f$withCurrent":I
    invoke-virtual {v7}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->getFirstStateRecord()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v9

    check-cast v9, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    check-cast v9, Landroidx/compose/runtime/snapshots/StateRecord;

    .local v9, "$this$withCurrent$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    const/4 v10, 0x0

    .line 305
    .local v10, "$i$f$withCurrent":I
    sget-object v11, Landroidx/compose/runtime/snapshots/Snapshot;->Companion:Landroidx/compose/runtime/snapshots/Snapshot$Companion;

    invoke-virtual {v11}, Landroidx/compose/runtime/snapshots/Snapshot$Companion;->getCurrent()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v11

    invoke-static {v9, v11}, Landroidx/compose/runtime/snapshots/SnapshotKt;->current(Landroidx/compose/runtime/snapshots/StateRecord;Landroidx/compose/runtime/snapshots/Snapshot;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v11

    check-cast v11, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    .local v11, "$this$mutate_u24lambda_u2d8$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    const/4 v12, 0x0

    .line 306
    .local v12, "$i$a$-withCurrent-SnapshotStateMap$mutate$1$iv$iv":I
    invoke-virtual {v11}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->getMap$runtime_release()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;

    move-result-object v13

    invoke-interface {v13}, Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;->builder()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;

    move-result-object v13

    .line 307
    .local v13, "builder$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;
    move-object v14, v13

    check-cast v14, Ljava/util/Map;

    .local v14, "it$iv":Ljava/util/Map;
    const/4 v15, 0x0

    .line 308
    .local v15, "$i$a$-mutate-SnapshotStateMap$removeIf$1$iv":I
    invoke-virtual {v3}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->entrySet()Ljava/util/Set;

    move-result-object v16

    invoke-interface/range {v16 .. v16}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object v16

    move/from16 v17, v0

    .end local v0    # "removed$iv":Z
    .local v17, "removed$iv":Z
    :goto_0
    invoke-interface/range {v16 .. v16}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_1

    invoke-interface/range {v16 .. v16}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/util/Map$Entry;

    .line 309
    .local v0, "entry$iv":Ljava/util/Map$Entry;
    move-object/from16 v18, v0

    .local v18, "it":Ljava/util/Map$Entry;
    const/16 v19, 0x0

    .line 204
    .local v19, "$i$a$-removeIf$runtime_release-SnapshotMapValueSet$removeAll$1":I
    invoke-interface/range {v18 .. v18}, Ljava/util/Map$Entry;->getValue()Ljava/lang/Object;

    move-result-object v1

    invoke-interface {v2, v1}, Ljava/util/Set;->contains(Ljava/lang/Object;)Z

    move-result v1

    .end local v18    # "it":Ljava/util/Map$Entry;
    .end local v19    # "$i$a$-removeIf$runtime_release-SnapshotMapValueSet$removeAll$1":I
    if-eqz v1, :cond_0

    .line 310
    invoke-interface {v0}, Ljava/util/Map$Entry;->getKey()Ljava/lang/Object;

    move-result-object v1

    invoke-interface {v14, v1}, Ljava/util/Map;->remove(Ljava/lang/Object;)Ljava/lang/Object;

    .line 311
    const/16 v17, 0x1

    move-object/from16 v1, p1

    .end local v0    # "entry$iv":Ljava/util/Map$Entry;
    goto :goto_0

    .line 204
    .restart local v0    # "entry$iv":Ljava/util/Map$Entry;
    :cond_0
    move-object/from16 v1, p1

    goto :goto_0

    .line 314
    .end local v0    # "entry$iv":Ljava/util/Map$Entry;
    :cond_1
    nop

    .end local v14    # "it$iv":Ljava/util/Map;
    .end local v15    # "$i$a$-mutate-SnapshotStateMap$removeIf$1$iv":I
    sget-object v1, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .line 315
    .local v1, "result$iv$iv":Ljava/lang/Object;
    invoke-interface {v13}, Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;->build()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;

    move-result-object v14

    .line 316
    .local v14, "newMap$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;
    invoke-virtual {v11}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->getMap$runtime_release()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;

    move-result-object v0

    if-eq v14, v0, :cond_2

    move-object v15, v5

    .local v15, "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/16 v16, 0x0

    .line 317
    .local v16, "$i$f$writable":I
    invoke-virtual {v15}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->getFirstStateRecord()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v0

    check-cast v0, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    move-object/from16 v18, v1

    .end local v1    # "result$iv$iv":Ljava/lang/Object;
    .local v18, "result$iv$iv":Ljava/lang/Object;
    move-object v1, v0

    check-cast v1, Landroidx/compose/runtime/snapshots/StateRecord;

    .local v1, "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    const/16 v19, 0x0

    .line 318
    .local v19, "$i$f$writable":I
    const/4 v0, 0x0

    .local v0, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getSnapshotInitializer()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v20

    .line 319
    .end local v0    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v20, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    const/16 v21, 0x0

    .line 320
    .local v21, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v22

    .local v22, "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    const/16 v23, 0x0

    .line 321
    .local v23, "$i$f$synchronized":I
    monitor-enter v22

    const/4 v0, 0x0

    .line 322
    .local v0, "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    :try_start_0
    sget-object v24, Landroidx/compose/runtime/snapshots/Snapshot;->Companion:Landroidx/compose/runtime/snapshots/Snapshot$Companion;

    invoke-virtual/range {v24 .. v24}, Landroidx/compose/runtime/snapshots/Snapshot$Companion;->getCurrent()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v24
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_3

    move-object/from16 v20, v24

    .line 323
    move/from16 v24, v0

    .end local v0    # "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    .local v24, "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    :try_start_1
    move-object v0, v15

    check-cast v0, Landroidx/compose/runtime/snapshots/StateObject;
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_2

    move-object/from16 v25, v2

    move-object/from16 v2, v20

    .end local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v2, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v25, "set":Ljava/util/Set;
    :try_start_2
    invoke-static {v1, v0, v2}, Landroidx/compose/runtime/snapshots/SnapshotKt;->writableRecord(Landroidx/compose/runtime/snapshots/StateRecord;Landroidx/compose/runtime/snapshots/StateObject;Landroidx/compose/runtime/snapshots/Snapshot;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v0

    check-cast v0, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    .local v0, "$this$mutate_u24lambda_u2d8_u24lambda_u2d7$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    const/16 v20, 0x0

    .line 324
    .local v20, "$i$a$-writable-SnapshotStateMap$mutate$1$1$iv$iv":I
    invoke-virtual {v0, v14}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->setMap$runtime_release(Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;)V

    .line 325
    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->getModification$runtime_release()I

    move-result v26
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_1

    move-object/from16 v27, v1

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v27, "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    add-int/lit8 v1, v26, 0x1

    :try_start_3
    invoke-virtual {v0, v1}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->setModification$runtime_release(I)V
    :try_end_3
    .catchall {:try_start_3 .. :try_end_3} :catchall_0

    .end local v0    # "$this$mutate_u24lambda_u2d8_u24lambda_u2d7$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    .end local v20    # "$i$a$-writable-SnapshotStateMap$mutate$1$1$iv$iv":I
    .end local v24    # "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    monitor-exit v22

    .line 321
    nop

    .line 320
    .end local v22    # "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v23    # "$i$f$synchronized":I
    nop

    .line 326
    .end local v21    # "$i$f$sync":I
    move/from16 v0, v26

    .local v0, "it$iv$iv$iv$iv":I
    const/4 v1, 0x0

    .line 327
    .local v1, "$i$a$-also-SnapshotKt$writable$4$iv$iv$iv$iv":I
    move/from16 v20, v0

    .end local v0    # "it$iv$iv$iv$iv":I
    .local v20, "it$iv$iv$iv$iv":I
    move-object v0, v15

    check-cast v0, Landroidx/compose/runtime/snapshots/StateObject;

    invoke-static {v2, v0}, Landroidx/compose/runtime/snapshots/SnapshotKt;->notifyWrite(Landroidx/compose/runtime/snapshots/Snapshot;Landroidx/compose/runtime/snapshots/StateObject;)V

    .line 328
    nop

    .line 326
    .end local v1    # "$i$a$-also-SnapshotKt$writable$4$iv$iv$iv$iv":I
    .end local v20    # "it$iv$iv$iv$iv":I
    nop

    .line 319
    nop

    .line 317
    .end local v2    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v19    # "$i$f$writable":I
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    goto :goto_2

    .line 325
    .restart local v2    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .restart local v19    # "$i$f$writable":I
    .restart local v21    # "$i$f$sync":I
    .restart local v22    # "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    .restart local v23    # "$i$f$synchronized":I
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    :catchall_0
    move-exception v0

    move-object/from16 v20, v2

    goto :goto_1

    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v1, "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    :catchall_1
    move-exception v0

    move-object/from16 v27, v1

    move-object/from16 v20, v2

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    goto :goto_1

    .end local v25    # "set":Ljava/util/Set;
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v2, "set":Ljava/util/Set;
    .local v20, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    :catchall_2
    move-exception v0

    move-object/from16 v27, v1

    move-object/from16 v25, v2

    move-object/from16 v2, v20

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v2, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .restart local v25    # "set":Ljava/util/Set;
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    goto :goto_1

    .end local v25    # "set":Ljava/util/Set;
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v2, "set":Ljava/util/Set;
    .restart local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    :catchall_3
    move-exception v0

    move-object/from16 v27, v1

    move-object/from16 v25, v2

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v2    # "set":Ljava/util/Set;
    .restart local v25    # "set":Ljava/util/Set;
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    :goto_1
    monitor-exit v22

    throw v0

    .line 316
    .end local v15    # "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v16    # "$i$f$writable":I
    .end local v18    # "result$iv$iv":Ljava/lang/Object;
    .end local v19    # "$i$f$writable":I
    .end local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v21    # "$i$f$sync":I
    .end local v22    # "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v23    # "$i$f$synchronized":I
    .end local v25    # "set":Ljava/util/Set;
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v1, "result$iv$iv":Ljava/lang/Object;
    .restart local v2    # "set":Ljava/util/Set;
    :cond_2
    move-object/from16 v18, v1

    move-object/from16 v25, v2

    .line 329
    .end local v1    # "result$iv$iv":Ljava/lang/Object;
    .end local v2    # "set":Ljava/util/Set;
    .restart local v18    # "result$iv$iv":Ljava/lang/Object;
    .restart local v25    # "set":Ljava/util/Set;
    :goto_2
    nop

    .line 304
    .end local v9    # "$this$withCurrent$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v10    # "$i$f$withCurrent":I
    .end local v11    # "$this$mutate_u24lambda_u2d8$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    .end local v12    # "$i$a$-withCurrent-SnapshotStateMap$mutate$1$iv$iv":I
    .end local v13    # "builder$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;
    .end local v14    # "newMap$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;
    .end local v18    # "result$iv$iv":Ljava/lang/Object;
    nop

    .line 330
    .end local v7    # "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v8    # "$i$f$withCurrent":I
    nop

    .line 331
    .end local v5    # "this_$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v6    # "$i$f$mutate":I
    nop

    .line 204
    .end local v3    # "this_$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v4    # "$i$f$removeIf$runtime_release":I
    .end local v17    # "removed$iv":Z
    return v17
.end method

.method public retainAll(Ljava/util/Collection;)Z
    .locals 28
    .param p1, "elements"    # Ljava/util/Collection;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Ljava/util/Collection<",
            "+",
            "Ljava/lang/Object;",
            ">;)Z"
        }
    .end annotation

    move-object/from16 v1, p1

    const-string v0, "elements"

    invoke-static {v1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 207
    move-object v0, v1

    check-cast v0, Ljava/lang/Iterable;

    invoke-static {v0}, Lkotlin/collections/CollectionsKt;->toSet(Ljava/lang/Iterable;)Ljava/util/Set;

    move-result-object v2

    .line 208
    .local v2, "set":Ljava/util/Set;
    invoke-virtual/range {p0 .. p0}, Landroidx/compose/runtime/snapshots/SnapshotMapValueSet;->getMap()Landroidx/compose/runtime/snapshots/SnapshotStateMap;

    move-result-object v3

    .local v3, "this_$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/4 v4, 0x0

    .line 332
    .local v4, "$i$f$removeIf$runtime_release":I
    const/4 v0, 0x0

    .line 333
    .local v0, "removed$iv":Z
    move-object v5, v3

    .local v5, "this_$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/4 v6, 0x0

    .line 334
    .local v6, "$i$f$mutate":I
    move-object v7, v5

    .local v7, "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/4 v8, 0x0

    .line 335
    .local v8, "$i$f$withCurrent":I
    invoke-virtual {v7}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->getFirstStateRecord()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v9

    check-cast v9, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    check-cast v9, Landroidx/compose/runtime/snapshots/StateRecord;

    .local v9, "$this$withCurrent$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    const/4 v10, 0x0

    .line 336
    .local v10, "$i$f$withCurrent":I
    sget-object v11, Landroidx/compose/runtime/snapshots/Snapshot;->Companion:Landroidx/compose/runtime/snapshots/Snapshot$Companion;

    invoke-virtual {v11}, Landroidx/compose/runtime/snapshots/Snapshot$Companion;->getCurrent()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v11

    invoke-static {v9, v11}, Landroidx/compose/runtime/snapshots/SnapshotKt;->current(Landroidx/compose/runtime/snapshots/StateRecord;Landroidx/compose/runtime/snapshots/Snapshot;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v11

    check-cast v11, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    .local v11, "$this$mutate_u24lambda_u2d8$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    const/4 v12, 0x0

    .line 337
    .local v12, "$i$a$-withCurrent-SnapshotStateMap$mutate$1$iv$iv":I
    invoke-virtual {v11}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->getMap$runtime_release()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;

    move-result-object v13

    invoke-interface {v13}, Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;->builder()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;

    move-result-object v13

    .line 338
    .local v13, "builder$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;
    move-object v14, v13

    check-cast v14, Ljava/util/Map;

    .local v14, "it$iv":Ljava/util/Map;
    const/4 v15, 0x0

    .line 339
    .local v15, "$i$a$-mutate-SnapshotStateMap$removeIf$1$iv":I
    invoke-virtual {v3}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->entrySet()Ljava/util/Set;

    move-result-object v16

    invoke-interface/range {v16 .. v16}, Ljava/util/Set;->iterator()Ljava/util/Iterator;

    move-result-object v16

    move/from16 v17, v0

    .end local v0    # "removed$iv":Z
    .local v17, "removed$iv":Z
    :goto_0
    invoke-interface/range {v16 .. v16}, Ljava/util/Iterator;->hasNext()Z

    move-result v0

    if-eqz v0, :cond_1

    invoke-interface/range {v16 .. v16}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/util/Map$Entry;

    .line 340
    .local v0, "entry$iv":Ljava/util/Map$Entry;
    move-object/from16 v18, v0

    .local v18, "it":Ljava/util/Map$Entry;
    const/16 v19, 0x0

    .line 208
    .local v19, "$i$a$-removeIf$runtime_release-SnapshotMapValueSet$retainAll$1":I
    invoke-interface/range {v18 .. v18}, Ljava/util/Map$Entry;->getValue()Ljava/lang/Object;

    move-result-object v1

    invoke-interface {v2, v1}, Ljava/util/Set;->contains(Ljava/lang/Object;)Z

    move-result v1

    .end local v18    # "it":Ljava/util/Map$Entry;
    .end local v19    # "$i$a$-removeIf$runtime_release-SnapshotMapValueSet$retainAll$1":I
    xor-int/lit8 v1, v1, 0x1

    if-eqz v1, :cond_0

    .line 341
    invoke-interface {v0}, Ljava/util/Map$Entry;->getKey()Ljava/lang/Object;

    move-result-object v1

    invoke-interface {v14, v1}, Ljava/util/Map;->remove(Ljava/lang/Object;)Ljava/lang/Object;

    .line 342
    const/16 v17, 0x1

    move-object/from16 v1, p1

    .end local v0    # "entry$iv":Ljava/util/Map$Entry;
    goto :goto_0

    .line 208
    .restart local v0    # "entry$iv":Ljava/util/Map$Entry;
    :cond_0
    move-object/from16 v1, p1

    goto :goto_0

    .line 345
    .end local v0    # "entry$iv":Ljava/util/Map$Entry;
    :cond_1
    nop

    .end local v14    # "it$iv":Ljava/util/Map;
    .end local v15    # "$i$a$-mutate-SnapshotStateMap$removeIf$1$iv":I
    sget-object v1, Lkotlin/Unit;->INSTANCE:Lkotlin/Unit;

    .line 346
    .local v1, "result$iv$iv":Ljava/lang/Object;
    invoke-interface {v13}, Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;->build()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;

    move-result-object v14

    .line 347
    .local v14, "newMap$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;
    invoke-virtual {v11}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->getMap$runtime_release()Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;

    move-result-object v0

    if-eq v14, v0, :cond_2

    move-object v15, v5

    .local v15, "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    const/16 v16, 0x0

    .line 348
    .local v16, "$i$f$writable":I
    invoke-virtual {v15}, Landroidx/compose/runtime/snapshots/SnapshotStateMap;->getFirstStateRecord()Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v0

    check-cast v0, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    move-object/from16 v18, v1

    .end local v1    # "result$iv$iv":Ljava/lang/Object;
    .local v18, "result$iv$iv":Ljava/lang/Object;
    move-object v1, v0

    check-cast v1, Landroidx/compose/runtime/snapshots/StateRecord;

    .local v1, "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    const/16 v19, 0x0

    .line 349
    .local v19, "$i$f$writable":I
    const/4 v0, 0x0

    .local v0, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getSnapshotInitializer()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v20

    .line 350
    .end local v0    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v20, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    const/16 v21, 0x0

    .line 351
    .local v21, "$i$f$sync":I
    invoke-static {}, Landroidx/compose/runtime/snapshots/SnapshotKt;->getLock()Ljava/lang/Object;

    move-result-object v22

    .local v22, "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    const/16 v23, 0x0

    .line 352
    .local v23, "$i$f$synchronized":I
    monitor-enter v22

    const/4 v0, 0x0

    .line 353
    .local v0, "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    :try_start_0
    sget-object v24, Landroidx/compose/runtime/snapshots/Snapshot;->Companion:Landroidx/compose/runtime/snapshots/Snapshot$Companion;

    invoke-virtual/range {v24 .. v24}, Landroidx/compose/runtime/snapshots/Snapshot$Companion;->getCurrent()Landroidx/compose/runtime/snapshots/Snapshot;

    move-result-object v24
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_3

    move-object/from16 v20, v24

    .line 354
    move/from16 v24, v0

    .end local v0    # "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    .local v24, "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    :try_start_1
    move-object v0, v15

    check-cast v0, Landroidx/compose/runtime/snapshots/StateObject;
    :try_end_1
    .catchall {:try_start_1 .. :try_end_1} :catchall_2

    move-object/from16 v25, v2

    move-object/from16 v2, v20

    .end local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v2, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v25, "set":Ljava/util/Set;
    :try_start_2
    invoke-static {v1, v0, v2}, Landroidx/compose/runtime/snapshots/SnapshotKt;->writableRecord(Landroidx/compose/runtime/snapshots/StateRecord;Landroidx/compose/runtime/snapshots/StateObject;Landroidx/compose/runtime/snapshots/Snapshot;)Landroidx/compose/runtime/snapshots/StateRecord;

    move-result-object v0

    check-cast v0, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;

    .local v0, "$this$mutate_u24lambda_u2d8_u24lambda_u2d7$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    const/16 v20, 0x0

    .line 355
    .local v20, "$i$a$-writable-SnapshotStateMap$mutate$1$1$iv$iv":I
    invoke-virtual {v0, v14}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->setMap$runtime_release(Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;)V

    .line 356
    invoke-virtual {v0}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->getModification$runtime_release()I

    move-result v26
    :try_end_2
    .catchall {:try_start_2 .. :try_end_2} :catchall_1

    move-object/from16 v27, v1

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v27, "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    add-int/lit8 v1, v26, 0x1

    :try_start_3
    invoke-virtual {v0, v1}, Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;->setModification$runtime_release(I)V
    :try_end_3
    .catchall {:try_start_3 .. :try_end_3} :catchall_0

    .end local v0    # "$this$mutate_u24lambda_u2d8_u24lambda_u2d7$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    .end local v20    # "$i$a$-writable-SnapshotStateMap$mutate$1$1$iv$iv":I
    .end local v24    # "$i$a$-sync-SnapshotKt$writable$3$iv$iv$iv$iv":I
    monitor-exit v22

    .line 352
    nop

    .line 351
    .end local v22    # "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v23    # "$i$f$synchronized":I
    nop

    .line 357
    .end local v21    # "$i$f$sync":I
    move/from16 v0, v26

    .local v0, "it$iv$iv$iv$iv":I
    const/4 v1, 0x0

    .line 358
    .local v1, "$i$a$-also-SnapshotKt$writable$4$iv$iv$iv$iv":I
    move/from16 v20, v0

    .end local v0    # "it$iv$iv$iv$iv":I
    .local v20, "it$iv$iv$iv$iv":I
    move-object v0, v15

    check-cast v0, Landroidx/compose/runtime/snapshots/StateObject;

    invoke-static {v2, v0}, Landroidx/compose/runtime/snapshots/SnapshotKt;->notifyWrite(Landroidx/compose/runtime/snapshots/Snapshot;Landroidx/compose/runtime/snapshots/StateObject;)V

    .line 359
    nop

    .line 357
    .end local v1    # "$i$a$-also-SnapshotKt$writable$4$iv$iv$iv$iv":I
    .end local v20    # "it$iv$iv$iv$iv":I
    nop

    .line 350
    nop

    .line 348
    .end local v2    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v19    # "$i$f$writable":I
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    goto :goto_2

    .line 356
    .restart local v2    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .restart local v19    # "$i$f$writable":I
    .restart local v21    # "$i$f$sync":I
    .restart local v22    # "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    .restart local v23    # "$i$f$synchronized":I
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    :catchall_0
    move-exception v0

    move-object/from16 v20, v2

    goto :goto_1

    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v1, "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    :catchall_1
    move-exception v0

    move-object/from16 v27, v1

    move-object/from16 v20, v2

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    goto :goto_1

    .end local v25    # "set":Ljava/util/Set;
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v2, "set":Ljava/util/Set;
    .local v20, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    :catchall_2
    move-exception v0

    move-object/from16 v27, v1

    move-object/from16 v25, v2

    move-object/from16 v2, v20

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .local v2, "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .restart local v25    # "set":Ljava/util/Set;
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    goto :goto_1

    .end local v25    # "set":Ljava/util/Set;
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .restart local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v2, "set":Ljava/util/Set;
    .restart local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    :catchall_3
    move-exception v0

    move-object/from16 v27, v1

    move-object/from16 v25, v2

    .end local v1    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v2    # "set":Ljava/util/Set;
    .restart local v25    # "set":Ljava/util/Set;
    .restart local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    :goto_1
    monitor-exit v22

    throw v0

    .line 347
    .end local v15    # "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v16    # "$i$f$writable":I
    .end local v18    # "result$iv$iv":Ljava/lang/Object;
    .end local v19    # "$i$f$writable":I
    .end local v20    # "snapshot$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v21    # "$i$f$sync":I
    .end local v22    # "lock$iv$iv$iv$iv$iv$iv":Ljava/lang/Object;
    .end local v23    # "$i$f$synchronized":I
    .end local v25    # "set":Ljava/util/Set;
    .end local v27    # "$this$writable$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .local v1, "result$iv$iv":Ljava/lang/Object;
    .restart local v2    # "set":Ljava/util/Set;
    :cond_2
    move-object/from16 v18, v1

    move-object/from16 v25, v2

    .line 360
    .end local v1    # "result$iv$iv":Ljava/lang/Object;
    .end local v2    # "set":Ljava/util/Set;
    .restart local v18    # "result$iv$iv":Ljava/lang/Object;
    .restart local v25    # "set":Ljava/util/Set;
    :goto_2
    nop

    .line 335
    .end local v9    # "$this$withCurrent$iv$iv$iv$iv":Landroidx/compose/runtime/snapshots/StateRecord;
    .end local v10    # "$i$f$withCurrent":I
    .end local v11    # "$this$mutate_u24lambda_u2d8$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord;
    .end local v12    # "$i$a$-withCurrent-SnapshotStateMap$mutate$1$iv$iv":I
    .end local v13    # "builder$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap$Builder;
    .end local v14    # "newMap$iv$iv":Landroidx/compose/runtime/external/kotlinx/collections/immutable/PersistentMap;
    .end local v18    # "result$iv$iv":Ljava/lang/Object;
    nop

    .line 361
    .end local v7    # "this_$iv$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v8    # "$i$f$withCurrent":I
    nop

    .line 362
    .end local v5    # "this_$iv$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v6    # "$i$f$mutate":I
    nop

    .line 208
    .end local v3    # "this_$iv":Landroidx/compose/runtime/snapshots/SnapshotStateMap;
    .end local v4    # "$i$f$removeIf$runtime_release":I
    .end local v17    # "removed$iv":Z
    return v17
.end method
